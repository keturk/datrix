...
WHY ``ValidationLevel.FAST``, NOT THE CLI'S ``STANDARD``
--------------------------------------------------------
The pipeline is driven at ``FAST``: every byte-shaping hook still runs
(``fix_imports`` and ``format_files`` -- ruff, CSharpier, google-java-format),
and only ``validate_files`` is skipped. ``validate_files`` is the compile pass
-- ``tsc --noEmit`` over a freshly installed ``node_modules``, ``mvnw compile``
in an emulated JVM, ``dotnet build`` with its resident compiler server -- and
it is read-only by the pipeline's own contract (see
``datrix_cli.pipeline.stages.language_hooks``): it changes no generated byte,
and everything it writes lands in a build directory :data:`EXCLUDED_DIRS`
already drops from the manifest. Running it here therefore bought nothing the
comparison could see, at the cost of a compiler per (example, language) pair
and a full ``node_modules`` tree per blessed example kept inside the editor's
watched workspace -- enough, across a corpus bless, to exhaust the reference
machine. Whether generated output COMPILES is proven where the toolchain lives:
each language package's own hook tests, and ``generate.ps1`` at ``STANDARD``.
``body_wire_naming_conformance`` and ``documentation_realization_parity`` drive
the pipeline at ``FAST`` for the same reason.
...
#: Line ending for every committed file this module writes (baseline manifests,
#: the blessed-count pin). Passed explicitly to ``write_text``: the default is
#: the platform's, which on Windows turns every ``\n`` into ``\r\n``, so a bless
#: on this machine wrote CRLF into files the repository normalizes to LF
#: (``.gitattributes`` ``* text=auto eol=lf``). Git then rewrote them on the next
#: touch, warned about it on every command, and ``git diff`` showed every line
#: of a re-blessed manifest as changed even when only a handful of hashes had.
_LINE_ENDING: str = "\n"
...
def load_known_non_generating() -> dict[str, str]:
    ...
    Raises:
        ValueError: If the file is missing, malformed, has an empty reason, or
            names an unregistered language in a qualified key.
    """
...
def write_blessed_count(count: int, path: Path = BLESSED_COUNT_PATH) -> None:
    ...
    path.write_text(
        json.dumps(payload, indent=2) + "\n", encoding="utf-8", newline=_LINE_ENDING
    )
...
PARKED_PAIR_NOW_GENERATES_MESSAGE: str = (
    ...
    "parity-known-nongenerating.json and bless with regen-parity-baselines.ps1"
)
...
#: Where the pipeline persists a migration ledger, relative to the application
#: source directory. Kept as one literal because the check below is the seam
#: between "the config says no ledger" and "the pipeline wrote none".
_LEDGER_DIR_RELPATH: Path = Path(".datrix") / "rdbms-migrations"


def _assert_no_ledger_left_beside_source(system_dtrx: Path, language: str) -> None:
    """Fail if generating *system_dtrx* persisted a migration ledger beside it.

    An example keeps no ledger: it is generated in every registered language and
    only ever meets a fresh database, and a persisted ledger records exactly one
    language's rendering of each revision -- the first language to generate would
    seal the store, and every later language in this same sweep would be refused
    at the foreign-language-ledger guard. The pipeline writes nothing when the
    example's system config declares ``migrations { ledger = false; }``; this is
    the check that the declaration and the behaviour agree, run after every
    generation rather than discovered as a mysterious refusal on the second
    language.

    Raises:
        RuntimeError: Naming the example, the language that wrote the ledger,
            and the config fix.
    """
    ledger_dir = system_dtrx.parent / _LEDGER_DIR_RELPATH
    if not ledger_dir.exists():
        return
    raise RuntimeError(
        f"Generating {system_dtrx.parent.name} in {language} persisted a migration "
        f"ledger at {ledger_dir}. An example never keeps a ledger: it is generated in "
        "every registered language, and a persisted ledger records one language's "
        "rendering of each revision, sealing the store against every other language. "
        "Fix: declare `migrations { ledger = false; }` in the example's system config "
        "(next to `enabled = true`), then delete that directory."
    )


def run_ledger_residue_self_test() -> list[str]:
    """Prove the no-ledger check fires on a planted ledger and stays quiet without one.

    Entirely in a PID-scoped scratch tree, never against a real example: a
    synthetic ``system.dtrx`` with a planted ``.datrix/rdbms-migrations/``
    beside it must be refused, naming the language and the config fix; the
    same file with nothing beside it must pass.

    Returns:
        Problem descriptions; empty means the check is sound.
    """
    problems: list[str] = []
    scratch = SCRATCH_ROOT / f"_ledger_residue_self_test-{os.getpid()}"
    _remove_tree(scratch)
    example_dir = scratch / "self-test-example"
    example_dir.mkdir(parents=True)
    dtrx = example_dir / "system.dtrx"
    dtrx.write_text("", encoding="utf-8")

    try:
        _assert_no_ledger_left_beside_source(dtrx, "python")
    except RuntimeError as exc:
        problems.append(f"self-test: the no-ledger check fired with no ledger present: {exc}")

    (example_dir / _LEDGER_DIR_RELPATH).mkdir(parents=True)
    try:
        _assert_no_ledger_left_beside_source(dtrx, "python")
    except RuntimeError as exc:
        message = str(exc)
        if "python" not in message or "ledger = false" not in message:
            problems.append(
                "self-test: the no-ledger refusal names neither the language nor the "
                f"config fix: {message}"
            )
    else:
        problems.append(
            "self-test: a planted .datrix/rdbms-migrations beside a system.dtrx was NOT "
            "refused -- the no-ledger check cannot fire"
        )
...
def generate_example(system_dtrx: Path, output_dir: Path, language: str) -> None:
    ...
    no incremental, no migrations), except ``target_language`` which is threaded
    from *language* exactly as ``datrix generate --language`` resolves it, and
    ``validation_level``, which is ``FAST`` rather than the CLI's ``STANDARD``
    (see the module docstring: the compile hooks ``STANDARD`` adds are
    read-only and can never move a byte this gate compares).
    ...
    *output_dir* (see :func:`_publish_generated_tree`) -- so a concurrent
    ...
    try:
        ...
        _assert_no_ledger_left_beside_source(system_dtrx, language)
    except Exception:
        _remove_tree(private_dir)
        raise
...
def cmd_bless(example_filter: str | None) -> int:
    ...
    for dtrx in examples:
        ...
        for language in languages:
            ...
            dest.write_text(
                manifest_to_text(manifest), encoding="utf-8", newline=_LINE_ENDING
            )
            blessed += 1
    ...
    if failures:
        print(
            ...
            f"if the defect is pre-existing and tracked."
        )
        return EXIT_FAIL
...
def cmd_check(example_filter: str | None) -> int:
    ...
    ledger_residue_problems = run_ledger_residue_self_test()
    if ledger_residue_problems:
        print("NO-LEDGER SELF-TEST FAILED:")
        for p in ledger_residue_problems:
            print(f"  {p}")
        return EXIT_FAIL
    print("no-ledger self-test: PASS (a planted ledger beside an example is refused, an absent one is not)")
