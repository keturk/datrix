...
**Parameters:** `-Warn`, `-ShowFiles`, `-BaseDir`, `-CheckTargetLiterals`, `-UpdateBaseline`, `-CheckProviderConditionals`, `-CheckFunctionLevelImports`, `-CheckSharedVocabulary`, `-CheckSharedTargetNames`, `-SelfTest`, `-Dbg`

**Exit codes:** 0 = clean (or warning mode), 1 = violations found (import-boundary, I1 target-literal ratchet, I6 provider-conditional ratchet, function-level-import ratchet, shared-vocabulary ratchet, shared-target-name ratchet, or a self-test failure), 2 = usage/config error (including a missing baseline file when `-CheckTargetLiterals`, `-CheckProviderConditionals`, `-CheckFunctionLevelImports`, `-CheckSharedVocabulary`, or `-CheckSharedTargetNames` is passed without having frozen one yet)
