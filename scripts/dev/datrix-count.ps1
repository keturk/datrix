...
.DESCRIPTION
 Counts the number of files with .dtrx and .dtrx.false extensions under subfolders of every datrix
repository discovered by Get-DatrixDirectories (the datrix showcase repo plus every datrix-* package
directory in the workspace). The set is discovered from disk, never hardcoded, so a new package is
