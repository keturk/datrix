/**
 * Resilient HTTP client for examples.IngestionService.
 *
 * Auto-generated by datrix-codegen-typescript from resilience.yaml config.
 * Do not edit manually.
 *
 * Timeout: 10000ms (override: EXAMPLES_INGESTION_SERVICE_TIMEOUT_MS)
...
const TIMEOUT_ENV = 'EXAMPLES_INGESTION_SERVICE_TIMEOUT_MS';
const TIMEOUT_MS = parseInt(process.env[TIMEOUT_ENV] ?? '', 10) || DEFAULT_TIMEOUT_MS;
const SERVICE_URL_ENV = 'EXAMPLES_INGESTION_SERVICE_SERVICE_URL';
const DEFAULT_URL = 'http://localhost:8000';
const DEPENDENCY_AUDIENCE = 'examples.IngestionService';
...
@Injectable()
export class IngestionServiceClientService {
  private readonly logger = new Logger(IngestionServiceClientService.name);
  ...
  /**
   * Make a resilient HTTP request to examples.IngestionService.
  ...
  async request(
    ...
    this.logger.debug(`examples.IngestionService request: ${method} ${url}`);
    ...
    const makeCall = async (): Promise<AxiosResponse> => {
      ...
      try {
        ...
        if (err.name === 'CanceledError' || err.name === 'AbortError') {
          throw new Error(`Request to examples.IngestionService timed out after ${TIMEOUT_MS}ms`);
        }
        throw error;
      } finally {
