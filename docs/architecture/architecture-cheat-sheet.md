...
| I6 | Language packages carry no NEW provider conditionals beyond the frozen baseline | `powershell -File "d:/datrix/datrix/scripts/dev/check-import-boundaries.ps1" -CheckProviderConditionals` — ratchet only (`datrix/scripts/config/provider-conditional-baseline.toml`); the plain string-literal provider-comparison sites in dotnet's and java's search-credential resolution are migrated onto the search-realization seam and frozen at zero; any remaining sites stay frozen until migrated |
...
- Declaration-driven gateway realizations (NGINX and Azure APIM), emitted when the system declares `gateway { }`; both consume the same shared route enumeration (`datrix_common.generation.gateway_routes`), so the public surface — including relationship-derived nested sub-collection routes — and per-route JWT enforcement can never diverge between them. Per-service routing derived from auth contracts; upstreams, health aliases, CORS, rate limit zones
...
Python 3.11+, Tree-sitter, Pydantic v2, Jinja2, ruff/CSharpier/google-java-format, mypy strict, pytest.
