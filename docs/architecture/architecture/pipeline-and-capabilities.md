...
> **Migration note:** `--language` is a required CLI argument, not a legacy override — it resolves in the `apply_cli_overrides` stage after service and infrastructure YAML are resolved on the AST (and after optional `--service` filtering), and before `platform_validation`. The legacy `--hosting` and `--platform` overrides are removed (see above).
