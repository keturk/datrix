...
The `datrix generate` command requires `--language`/`-L` (resolved against the registered `datrix.languages` set via `resolve_language_id`) and reads `deployment` (runtime, provider, target, registry) from resolved config. Deployment-affecting CLI overrides (`--hosting`, `--platform`) are removed — see [Decision 6: Deployment Target Contract](../architecture-overview.md#decision-6-deployment-target-contract-stable). The pipeline validates deployment dimensions (runtime/provider/target/infrastructure-flavor combinations) before generation starts. Service deployment shape is derived from declared DSL blocks, not from a per-service runtime-flavor selector — see [Design Principles — Construct-Mapped Platform Realization](../design-principles.md#11-construct-mapped-platform-realization-stable).

> **Migration note:** `--language` is a required CLI argument, not a legacy override — it is resolved against the registered `datrix.languages` set before the pipeline runs and selects the backend language plugin; there is no `apply_cli_overrides` stage and no `platform_validation` stage. The legacy `--hosting` and `--platform` overrides are removed (see above).

**Pipeline stages (`GenerationPipeline.run` in `datrix-cli`, `datrix-cli/src/datrix_cli/pipeline/generation.py`)** are an ordered registry of typed `Stage` values executed through one runner. In order: `parse` → `discover_and_parse_seeds` → `resolve_service_configs` → `inject_identity_system_entities` → `analyze` (which also runs infrastructure-config resolution and service memory-limit normalization as pre-seal hooks, because both depend on nodes analysis may synthesize) → `apply_service_filter` (skipped without `--service`) → `validate_deployment` (runtime/provider/target/infrastructure-flavor combinations) → `resolve_incremental` (may early-exit when nothing changed) → `build_codegen_context` → `build_deployment_plan` (resolves `DeploymentPlan` with language generators, runtime generators, and provider generators from the deployment config — see [Decision 6: Deployment Target Contract](../architecture-overview.md#decision-6-deployment-target-contract-stable)) → `resolve_config_surfaces` → `discover_generators` → `validate_type_completeness` → `discover_platforms` (receives pre-resolved platform config from the plan) → `sort_generators` → `select_generation_targets` (skipped without `--only`) → `attach_runtime_bootstrap` → one `generate:{name}` stage per generator → `resolve_intra_group_conflicts` → `detect_file_conflicts` → `assert_deploy_binding_conformance` → one `write:{target}` stage per target → `migrations` → `discover_language_hooks` → `run_language_post_processing` (gated by `validation_level`) → `format_json_files` → `update_append_only_hashes` → `snapshot` → `commit_migration_state` (writes the migration-state commits every generator staged in the run's `MigrationStateTransaction`; skipped for a dry run, an `--only` run, or a run that recorded a failure, so none of those advances a ledger). Extension **directives** are recorded during parse; **registry** and **type-registry** integration run when the active code path invokes `PluginRegistry` / `TypeRegistry` APIs (see [Domain extension system](repository-architecture.md#domain-extension-system)). Language generators receive declared extension names via `declared_extension_names(app)` and merge per-language maps through the shared `build_type_map` (for example `datrix-codegen-python` calling it with its own `PYTHON_TYPE_MAP`/`PYTHON_EXTENSION_MAPS`).
...
| `standard` | Run all three hooks: `fix_imports`, `format_files`, `validate_files`. This is the default and the most thorough level — what `validate_files` checks is each language plugin's own decision (ruff for Python, `tsc --noEmit` for TypeScript, and so on), never a CLI-owned tool list. |
...
- **Multi-service NGINX gateway** (Stable) — When the system profile declares a `gateway { }` block, `datrix_codegen_docker.generators.config.gateway_generator` renders `config/nginx/nginx.conf` via `nginx.conf.j2`: per-service routing is derived from each service's resolved ingress exposure — one **upstream** per service deriving `GATEWAY` (a service whose endpoints are all `auth(service)` derives `INTERNAL` and is not routed), **location** blocks for public REST paths (including each `rest_api` **base_path** prefix), **GraphQL** `graphql_api` base paths (with WebSocket upgrade headers when the API has subscriptions), **health** aliases derived from the primary REST `base_path` plus `service.config.health_check.path` when a primary REST API exists (with `GenerationError` on duplicate external health paths), **403** for versioned paths matching internal REST segments (`INTERNAL_PATH_DENY_REGEX` in the gateway module), **CORS OPTIONS** handling and **proxy timeouts** on proxied locations, and **`client_max_body_size`** on routes for services that declare **storage** blocks. Rate limit zones from `@rateLimit` on endpoints are preserved. One route per AST-derived nested sub-collection route (`GET /{parent}/:id/{child}`, from `hasMany` relationships) is also emitted, using the same shared `datrix_common.generation.nested_collection_routes` derivation every language generator's own handler synthesis consumes, with JWT skip decided by the same per-route `PublicRoute.requires_jwt` flag every other route in this gateway uses — a parity gate holds the rendered location set equal to the shared route enumeration (`datrix_common.generation.gateway_routes.gateway_public_routes`), so this realization cannot silently drop or diverge from a route the enumeration publishes. If an upstream has **no** matching locations, generation raises `GenerationError` listing that upstream (instead of emitting a broken config). Test-only multi-service fixtures use `attach_config_for_docker` / `ensure_minimal_rest_api_for_multi_service_gateway` in `datrix_codegen_docker.test_helpers` to add minimal `rest_api` stubs when the parsed `.dtrx` omits API blocks.

- **Liveness / readiness / health split** (Stable) — Generated services expose three distinct probes instead of a single conflated `/health`: **`/live`** (process liveness only), **`/ready`** (all `required` dependencies usable — returns 503 when a required dependency is down), and **`/health`** (detailed per-dependency report, including `degraded` optional dependencies). A dependency declared `health = "degraded"` keeps `/ready` green while `/health` reports `degraded`, unless a per-dependency `readyOnDegraded = false` makes it a readiness blocker for guarded rollouts. Deployment probes (Docker healthcheck) point at `/ready`; the prior single-`/health` contract is replaced outright, with no backward-compat fields.
...
- **Rate limiting** (Stable) — A unified plan-based system requiring a Redis/Valkey-backed cache block:
  ...
  **No runtime bypass switch.** The service raises 503 when Redis is unavailable; there is no environment variable that disables rate limiting in service code. The only bypass lives in the emitted deploy-test harness (`project/_deploy_test_main.py.j2`), which is test infrastructure, not service runtime.
...
**Per-route JWT and audience enforcement (Azure APIM):** the APIM realization validates JWT per route rather than globally — a route the shared route enumeration marks public is exempt, and every other route is validated strictly after routing so an unmatched request never reaches the check. `<audiences>` is populated from the declared gateway audience plus one named-value reference per identity provider the system exposes to end users, so the gateway rejects a token issued for the wrong audience instead of accepting any token the tenant issues. See [datrix-codegen-azure architecture](../../../../datrix-codegen-azure/docs/architecture.md#apim-gateway-contract-realization) for the full mechanism.
...
**Cross-reference:**
- [Config System — serverGroup / namespaceGroup fields](../../../../datrix-common/docs/architecture/config-system.md#servergroup--namespacegroup--cachegroup--infrastructure-resource-pooling-fields)
...
## Managed Identity Provider Integration (Stable)

> Implemented — see [Decision 13](../architecture-overview.md#decision-13-managed-identity-provider-integration-adopted). The pipeline's `inject_identity_system_entities` stage runs before analysis, and the language generators emit the validators, guards and profile services named below (python: `templates/api/identity.py.j2`, `auth.py.j2`, `cross_cutting/trusted_caller_middleware.py.j2`; typescript: `templates/auth/identity.ts.j2` and its guards).
