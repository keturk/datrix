...
> **Scaffolding in progress:** `datrix-codegen-dotnet` is a registered workspace package whose repo exists and is already discovered by the toolchain, but whose source has not fully landed yet. It is counted above because it is a real package, not because it generates yet. Repo tooling keys off what is on disk: it joins `test.ps1 -All`, `mypy.ps1 -All`, `status-tests.ps1`, the shared-venv install set, and the import-boundary / dead-code / docs-conformance scans automatically once it has a `pyproject.toml`, `src/`, and `tests/`. No hand-maintained package list needs updating. `datrix-codegen-java` is no longer in this state — it is a fully realized generator at parity with python and typescript (see [datrix-codegen-java/docs/architecture.md](../../../datrix-codegen-java/docs/architecture.md)).
...
Generates .NET server applications: data models, API routes, request/response schemas, HTTP clients for service-to-service calls, infrastructure code for databases, caching, pub/sub, and resilience patterns, plus project scaffolding with entry points and dependencies. **Scaffolding in progress** — the repo is registered and discovered, but source has not fully landed yet.
...
Provider generators own the cloud infrastructure for provider-native runtimes. Where the docker platform declares a cloud runtime in its `container_scaffold_runtimes` (`ecs-fargate`, `app-runner`, `azure-app-service-container`), the docker generator supplies the per-service Dockerfiles that runtime packages into images — the provider generator still owns all infrastructure. The `docker-compose` runtime is local-only and is never paired with a cloud provider, so provider generators never augment Compose output. For `runtime: azure-app-service` (code-based, no containers), the Azure generator produces all infrastructure directly — there is no separate runtime generator and no Dockerfile involvement.
