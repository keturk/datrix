...
Generates Java code (Spring Boot). JPA/Hibernate persistence, Liquibase migrations. Per-service Maven project scaffolding (`pom.xml` pinned to Spring Boot 4.1 / Java 25, the mandatory `mvnw` wrapper, `Application.java` with the `--datrix.run=<mode>` single-image dispatch) has landed; the remaining parity surface (API routes, HTTP clients, caching/pub-sub/resilience infrastructure) lands in later increments.
