...
Generates .NET server applications: data models, API routes, request/response schemas, HTTP clients for service-to-service calls, infrastructure code for databases, caching, pub/sub, and resilience patterns, plus project scaffolding with entry points and dependencies. **Scaffolding in progress** — the repo is registered and discovered, but source has not landed yet.

#### 8. datrix-codegen-java
Generates Java code (Spring Boot). JPA/Hibernate persistence, Liquibase migrations. Per-service Maven project scaffolding (`pom.xml` pinned to Spring Boot 4.1 / Java 25, the mandatory `mvnw` wrapper, `Application.java` with the `--datrix.run=<mode>` single-image dispatch) lands in design 036 Phase 3 (task 40-19); the remaining parity surface (API routes, HTTP clients, caching/pub-sub/resilience infrastructure) lands in later phases per design 036's increments.
