...
> **Planned — `datrix-codegen-platform-common` (Design 010).** Design 010 (platform extensibility) introduces a `datrix-codegen-platform-common` package. When it lands, it joins this graph between `datrix-codegen-common` and the platform generators. It depends on **`datrix-common` *and* `datrix-codegen-common`** (not `datrix-common` alone), and inherits the same denied-subtree rule platform generators carry: it must not import the language-shaped parts of `datrix-codegen-common` (`transpiler.*`, language-shaped `context_models`/`algorithms`). The shared Grafana `DashboardBuilder` **stays in `datrix-codegen-common/dashboards/`**; `platform-common` exposes a thin `ObservabilityIntegration` facade that re-exports it (no physical move, so the `dashboards.*` allowed-subtree edge is unchanged). This is an **external prerequisite** tracked under Design 010; the boundary table here records whichever final edge Design 010 produces. See [Decision 12: Language-Agnostic Provider Generators](#decision-12-language-agnostic-provider-generators).
...
- **`datrix-codegen-platform-common`** is the shared home for provider-level concerns that are language-agnostic and shared by ≥2 platforms: the `resolve_runtime_spec(context)` discovery helper (raises `GenerationError`, never falls back to Python), the `runtime_stack_token(lang_spec, runtime_version)` `LANG|VERSION` composer, the `ObservabilityIntegration` facade re-exporting `DashboardBuilder`, and the `PlatformInfrastructure` protocol
- **`PlatformInfrastructure` protocol** (`@runtime_checkable`, in `platform-common`) expresses provider-level infrastructure surfaces — `network_topology(app)`, `service_to_service_auth(app)`, and `provision_managed_service(block, block_kind, service)` — exposed as a `platform_infrastructure` property on each `PlatformGenerator` subclass. Every platform implements the **full** protocol: clouds fully; Docker/K8s return explicit no-op value objects (`NetworkTopology.none()`, empty `ManagedServicePlan`) — honest "no VPC/IAM" facts, never silent stubs. Value objects (`NetworkTopology`, `ServiceAuthModel`, `ManagedServicePlan`) are frozen Pydantic models in `platform-common`, keeping provider concepts out of the AST model
- **The platform seam is the existing `PlatformGenerator` + `datrix.platforms` entry-point group** — discovered via `discover_platforms`. No new `PlatformAdapter` type is introduced. `PlatformInfrastructure` and `ObservabilityIntegration` are *consumed by* `PlatformGenerator` subclasses, never a competing discovery contract. A new provider implements a `PlatformGenerator` subclass + a `PlatformInfrastructure` implementation, and *consumes* the shared `ObservabilityIntegration` facade and `LanguageRuntimeSpec` — language support is free
