...
**Status:** Approved — Implementation In Progress. Landed and held by executable gates: claimed builtin groups compared across every registered language (`builtin-claims-parity-gate.ps1`), every registered language's stance over the full shared domain universe compared the same way (closure, completeness, divergence reported with its declared reason) (`supported-domain-parity-gate.ps1`), extension type-map completeness per language (`type-mapping-completeness.ps1`), provider-name conditionals in language packages held at an empty ratchet baseline (`check-import-boundaries.ps1` with `-CheckProviderConditionals`), and the reference-example parity baselines (`reference-example-parity-gate.ps1`). This decision moves to Adopted when every result bullet above is named here with the check that holds it and the parity baseline matrix carries no parked pair without a recorded reason; until then the heading says in progress.
...
| 10 | Parallel implementations across language packages, and separately across platform packages, are measurable by a signal that survives divergence | Superseded and deleted (Decision 47): the role-keyed `decision-parity-gate.ps1` compares decision skeletons grouped by role on both axes; it reproduced the retired name-keyed report's platform-axis output before that report and its wrapper were deleted |
...
**Rationale:**
- Parity **measurement** on the language axis is mature — two-axis drift ratchets, collapsibility classification, derived cross-language gates, typed exemption files with pinned counts — though Decision 47 later found that this measurement compared text, not decisions. Parity **prevention** is not. A feature still lands once in the shared model and then once by hand in every language package, and the gates report the resulting drift only after it exists. The exemption files are where that drift legally accumulates.
...
**Result:** four workstreams in dependency order. Everything in A is a gate or a fail-loud declaration and lands **before** anything it governs; A and B assert byte-identical generated output, so neither changes a generated byte.
...
- **B — a shared context builder and a typed context model per domain that is compared by input; per-language difference becomes a declared parameter, never a branch.** A domain compared by input has exactly one `build_<domain>_context` in the shared algorithms layer returning a frozen dataclass, registered in `_RICH_CONTEXT_TYPES` as that domain's parity context and held to a real production constructor by a hard-zero AST census; every other domain resolves to `None` and compares by artifact presence through the artifact-role gate — there is no digest fallback (the fingerprint context is deleted) and no pending ledger. Where a genuine per-language **fact** enters a shared builder (one language collecting imports eagerly through its type resolver, another's fixed foreign-key column convention, each ORM's spatial API name), it becomes a `LanguageProfile` value the shared builder reads, a typed parameter the language's thin adapter passes (Decision 38 D11's "caller supplies its own exception class" is the precedent), or a per-language render step over the shared context. It never becomes a target-name branch in the shared layer. **This is the rule for new work and the remedy for an observed defect, not a retroactive count target.** The retroactive application to the existing population is Decision 47, which brings every language's decisions to Python's before hoisting. A new cross-language feature is authored once as a shared plan; an existing per-language divergence is hoisted when it produces a defect, each hoist a byte-identical refactor proven on the reference examples with a per-symbol negative test that no language package redefines the private name. The original programme to hoist every group the collapsibility ledger had labelled with a mechanism was run to its end: reading the bodies showed most carried a per-language *decision* (SQLAlchemy columns versus JPA annotations versus EF Core POCOs share a name and an input, not an algorithm), the ledger's arithmetic did not survive the audit, and folding four implementations through one function with a parameter for every difference would have been the four implementations again with a wider signature.
...
| A1-3 | No two languages spell one emission with two domain ids, and every rename is output-neutral | Negative: the union report lists zero ids whose stance reason names another id as the same emission. Positive: every renamed language is byte-identical before and after |
...
| B-4 | Every hoist is byte-identical on the reference examples for every affected language | `reference-example-parity-gate.ps1`, exit 0 |
...
| C-5 | Migration is byte-identical | Byte-identity generation per language |
...
| 8 | Existing output is byte-identical | `datrix/scripts/test/reference-example-parity-gate.ps1` over its corpus example on every registered language |
...
### Decision 47: Language-Axis Decision Parity — Roles, Decision Skeletons, and Python as the Reference Target (Approved — Implementation In Progress)

**Rationale:**
- The retired text-keyed drift scanner compared verbatim source: byte-equal decorators, docstrings and bodies, which answers "does the text match" rather than "do the generators make the same decisions." The per-name classification ledger built on top of it held 550 entries — 547 `intentional`, 529 with collapsibility mechanism `none` — but at retirement 21 entries still carried a mechanism other than `none`, and 12 of those names still drift today. 28 entries' own written reasons said the bodies were byte-identical, yet were still ruled `intentional`, and 0 byte-identical groups were ever in the ledger or either baseline, because both counted only drifted groups. The ledger's verdicts were written by where a copy lived, not by whether the target language required the difference.
- Re-parsing the drifted population confirms text similarity is the wrong axis in both directions: 26 of 541 previously-drifted groups differ only in a docstring, a type annotation, or a local variable name, and were written up as intentional divergence anyway. Grouping by bare name also hides parallelism: stripping each language's own name tokens (`build_java_cqrs_command_context` and `build_ts_cqrs_command_context` becoming one role) exposes 195 further cross-language groups invisible to a name-keyed scan. Comparing **decision skeletons** — control flow, predicates and model-attribute chains, with every literal collapsed — over the resulting 774 role groups finds 31 with an identical body, 137 that make the same decisions but render differently, and 554 (a proxy-extractor upper bound) that decide differently over the same source construct.
- The CQRS command-handler role is the worked instance: `build_python_cqrs_command_context` (`datrix-codegen-python/src/datrix_codegen_python/orchestration/cqrs_context_builders.py`), `build_ts_cqrs_command_context` (`datrix-codegen-typescript/src/datrix_codegen_typescript/orchestration/cqrs_frozen_builders.py`), `build_java_cqrs_command_context` (`datrix-codegen-java/src/datrix_codegen_java/orchestration/cqrs_context_builders.py`) and `build_dotnet_cqrs_command_context` (`datrix-codegen-dotnet/src/datrix_codegen_dotnet/orchestration/cqrs_context_builders.py`) all consume the same `Command` node and construct the same frozen `CqrsCommandContext`, and still diverge on five concrete decisions: an empty command body makes java raise `GenerationError`, python transpile a `pass` handler, and typescript substitute a stub body; python alone inserts a completion log line before the last `return`; typescript alone injects null guards after a `findOne`; python alone derives a block-scoped session name from `command.block_name`; and typescript alone derives constructor dependencies from the entities the body used, while python returns them empty. The `cqrs` domain is compared by artifact presence only, so nothing today compares these four sets of decisions against each other.

**Result:** the comparison instrument is replaced first, then every language's decisions are brought to parity with Python, then each reconciled decision is hoisted so it exists exactly once.

- **D1 — The comparison unit is the role, not the name; the comparison is the decision skeleton, not the body.** The scanner (`datrix/scripts/library/test/decision_parity.py`, built on the skeleton extractor `datrix/scripts/library/test/decision_skeleton.py`, wrapped by `datrix/scripts/test/decision-parity-gate.ps1`) AST-walks every registered `datrix.languages` package's `src/` tree and groups functions into **roles** by two keys in order: a *signature role* — the tuple of parameter and return types drawn from `datrix_common`/`datrix_codegen_common`, with language-private parameter types dropped from the key — and, for functions with no shared-typed parameter, a *normalized-name role*, the bare name with underscore-delimited language tokens stripped. It then extracts each function's decision skeleton (`if`/`for`/`while`/comprehension/`return`/`raise` structure, model-rooted predicates and calls preserved, every literal and every other operand collapsed to a placeholder) and emits one verdict per multi-package role — `identical`, `same-decisions`, or `decision-divergence` — naming every member by `package:file:line`. The scanner proves its own non-vacuity every run against a synthetic tree covering all four buckets plus a token-split pair and a signature-unified pair, and refuses to run against fewer than two targets.
- **D2 — Two gates, both hard-zero, with one shape-exempt and one declared-exception path.** `identical` and `same-decisions` roles fail the gate outright; the only exempt shape is the pre-binding adapter (a body that is a single `return` of a call into `datrix_codegen_common`, passing through its own parameters plus this package's language id and casing callables), recognized by AST shape, never by a written exemption. `decision-divergence` roles fail unless every member other than the ones agreeing with Python declares the construct `unsupported(reason)` on its `LanguageCapabilityDeclaration`, read through the existing capability-resolution surface. Exceptions are keyed by role, never by name, so a rename never touches one. During migration the gate runs with a scope list of the domains already brought to parity, held in the gate's own config file; the list only grows, and is deleted once every domain is in scope and the gate runs unscoped. The same config file's `buckets` list independently gates every role of a named verdict (`identical`, `same-decisions`, `decision-divergence`) across every domain regardless of the domains list, and it too only grows.
- **D3 — Reconciliation rule: Python's decision, with a closed exception class.** For every `decision-divergence` role the other languages adopt Python's skeleton, unless the non-Python decision is a fail-closed validation Python lacks (java's empty-body raise is this case: python's `pass` fallback and typescript's stub body are deleted, not java's raise), a realization of a DSL-declared capability Python silently drops, or a stricter security posture (a guard, an escaping step, a narrower permission one language applies and Python does not). Nothing outside those three follows from a language's code being shorter, faster, or more idiomatic — "better implementation" reduces to one of the three cases or it follows Python. Where Python itself carries two internal variants of one decision, the variant reached from the production plugin path is the reference and the other is deleted.
- **D4 — Port, then hoist: every reconciled skeleton gets exactly one home.** Once every language in a role decides alike, context builders move to `build_<role>_context` in `datrix_codegen_common/algorithms/`, with the per-language leaf passed in as a `LanguageProfile` value, a typed callable from the language's thin adapter, or a per-language render step over the shared context — never a target-name branch. Pure predicates and lookups move to `service_predicates.py` or the owning `algorithms` module, parameterized by the caller's own exception class where they raise. Every hoist lands inside an already-declared dependency edge (`manifest-import-parity-gate.ps1`), registers the role's context type in `_RICH_CONTEXT_TYPES` when the micro-generator consumes it — `cqrs` moves from artifact-presence comparison to input comparison across its six per-item contexts — and carries the per-symbol pair of tests already used for prior hoists (no language package redefines the private name; every consuming package reaches the shared function) plus a positive test that the shared builder drives behaviour.
...
- **D6 — Proof of a port is regenerated output, re-blessed once per role.** `reference-example-parity-gate.ps1` fails for the languages whose decisions changed and passes for the ones that did not; the diff of the regenerated `01-foundation` tree for each changed language is read and shows exactly the reconciled decision and nothing else; `regen-parity-baselines.ps1` re-blesses, and the blessed-count ratchet never decreases; the subsequent hoist is byte-identical on every language (gate green, no re-bless), because parity was reached before the move. A role whose port produces no diff on `01-foundation` names the example under `datrix/examples/` that exercises the construct and blesses that example instead.
...
- **D10 — A role resolves to a domain id by a closed ladder, first hit wins.** The shared context type is reverse-looked-up in `_RICH_CONTEXT_TYPES` when it maps to exactly one non-test id; failing that, the context type's owning module basename (with a `_contexts`/`_context`/`_render_contexts` suffix or a `persistence_` prefix stripped) is checked against the universe ids; failing that, every member's source module (`micro_generators/<id>.py`, `hooks/<id>_hooks.py`, `orchestration/<id>_context_builders.py`/`<id>_frozen_builders.py`, `generators/<id>/`) must equal one universe id with every member agreeing; otherwise the role is `undomained`. Scoping accepts universe ids plus the literal `undomained`. A divergent role is exempt from the gate only when each lagging language's own `DomainDeclaration` for that domain is `unsupported`, the domain is in that language's `on_demand_domains`, or an `@emit_adapter` member's emit-table row resolves to a builtin group whose stance is `unsupported` — an `undomained` role admits only the last of the three. A member the resolver cannot parse or resolve is a failure naming that member, never a silent skip.

| # | Invariant | Check |
|---|---|---|
| I1 | Two language packages never carry the same decision skeleton | The decision-parity gate: `identical` and `same-decisions` roles fail; pre-binding adapters exempt by AST shape only |
| I2 | Two language packages never make different decisions over the same source construct unless the lagging one declares `unsupported(reason)` | Same gate, `decision-divergence` bucket, read against `LanguageCapabilityDeclaration` |
...
| I5 | Python is the reference; a non-Python decision wins only under D3's three cases | Each port's record names the case or states "follows Python"; the design-conformance gate rejects any other wording |
| I6 | A hoist follows parity, never precedes it | Every hoist depends on its role's port; reference-example gate green with no re-bless on the hoist commit |
...
| I9 | A port is proven by generated output | Reference-example gate red on the lagging language, diff read, re-blessed; blessed count never decreases |
| I10 | The gate proves its own non-vacuity every run | Five-bucket synthetic self-test, single-target refusal |
| I11 | No written reason ever records why two languages decide differently | No classification file exists; the only exception surface is the typed `unsupported(reason)` declaration on the plugin |

**Security posture:** this decision touches no trust boundary, secret, transport, or permission directly. Where a reconciled role sits on one — an auth-index builder, a sensitive-field collector, a hash-from-plaintext helper and a credential-resolution helper are all in the measured population — D3's third case binds: the decision that enforces more, guards more, or discloses less is the one every language adopts, whether or not it is Python's. The gate itself fails closed: a role it cannot classify (an unparseable member, an unresolvable annotation) is reported as a failure naming the member, never skipped. No generated default changes except toward the stricter decision.

**Non-goals:** restoring the classification ledger (it recorded location as justification, was keyed by name so renames churned it, never covered identical bodies, and its "all `none`" conclusion was already false when written); a body-similarity threshold as the gate (it would penalize exactly the rendering differences the architecture requires — similarity is used once, to order the migration worklist, never as a verdict); hoisting before reconciliation (it would freeze one language's drift into the shared layer); and folding four implementations through one function with a parameter per difference (that is the four implementations with a wider signature, rejected here as it was previously). Also out of scope: the platform axis and the platform-to-language protocol seam (D7); template bodies, which stay per language — only decisions and structure move to the shared layer; any new `LanguageProfile` role beyond what a specific port requires; and the SQL, component, and Angular codegen packages, which are not `datrix.languages` plugins.
...
2. The 31 identical-body roles are hoisted with a byte-identical proof and the per-symbol tests; scope gains each role's domain as it lands.
3. The 137 same-decisions roles are hoisted the same way, with rendering differences becoming declared parameters.
4. The CQRS role is ported — typescript, java and dotnet move to Python's decisions under D3's first case, java's raise becomes the shared decision, Python's fallback and typescript's stub are deleted — then re-blessed and hoisted as six shared builders, and `cqrs` enters `_RICH_CONTEXT_TYPES`.
5. The decision-divergence roles at skeleton similarity 0.75 or above are read against Python one at a time: ported, re-blessed, hoisted, renamed.
6. The remaining decision-divergence roles follow the same read-port-rebless-hoist-rename cycle; a role whose lagging language genuinely cannot realize the construct gets an `unsupported(reason)` declaration and a filed capability work item instead, expected to be rare.
...
bucket is now gated repo-wide via the decision-parity scope file's `buckets` key -- every
surviving `identical`-verdict role is a declared pre-binding adapter, none a genuine
duplicate. Same-decision hoists, the CQRS port-then-hoist, and the remaining reconciliations
follow in that order, each proven by regenerated output.
