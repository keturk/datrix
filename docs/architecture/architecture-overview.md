...
### Decision 44: Parity by Construction — Declared Universes, Shared Plan Modules, and Declared Routing on the Language Axis (Approved — Implementation In Progress)
...
- **B — one shared context builder per shared domain; per-language difference becomes a declared parameter, never a branch.** Every universe domain gets exactly one `build_<domain>_context` in the shared algorithms layer returning a frozen dataclass, registered as that domain's rich parity context; the fingerprint context is deleted when the last domain leaves it, and until then a domain is either rich or explicitly recorded as pending with the mechanism that will move it — no third state. Where a classification reason names a genuine per-language **fact** (one language collecting imports eagerly through its type resolver, another's fixed foreign-key column convention, each ORM's spatial API name), that fact becomes a `LanguageProfile` value the shared builder reads, a typed parameter the language's thin adapter passes (Decision 38 D11's "caller supplies its own exception class" is the precedent), or a per-language render step over the shared context. It never becomes a target-name branch in the shared layer. Every hoist is a **byte-identical refactor pinned in the same change**: the drift baseline decreases by exactly the groups the hoist removed and the classification file loses those entries, and a hoist that does not move the count is not accepted (Decision 38 D6). Each hoist begins by reading every language's body for the group and writing the difference census down before any edit — a group whose bodies diverge in *decision* rather than in target fact is reclassified to its real mechanism first, because **a mechanism label is a hypothesis about the code, not a fact about it** (Decision 38 D3).
- **C — every single-step routing decision is a declared row, and the table is the first dispatch stop, not a fallback.** Of the six known chain steps, three are single-step dispatch and three are multi-step lowering excluded by design. At the three single-step steps, every branch keying on a `(category, method)` pair plus a receiver shape, arity, literal argument, receiver type or flag becomes an `EmitDecl` row whose emit function carries the `@emit_adapter` marker; the predicate columns are used as they exist, and no new column is added without a measured branch no existing column can express, with that measurement recorded in the row module's docstring. At every call site the order is `emit_function_for` → declared adapter, else the ordinary mapping path, and a `visit_*` branch that duplicates a row's predicate is **deleted in the same change** — the emission-path gate's rule for GenDSL, applied here: a declaration replaces the branch, it never joins it. Each language's non-adapter visit floor is then lowered to its live count, and the transpiler-floor documentation names every remaining function for **all four** languages, so the dotnet/java addendum stops being tracked separately. A row for a builtin whose group the language declares `unsupported` is a registration error — a routing declaration for a capability the language says it lacks is a contradiction, and today it would be silent.
- **D — what A–C made unnecessary is retired, and the rest is pinned at its terminal value.** The per-method builtin mapping exemption file is deleted; the language-axis drift baseline and its classification file come to rest at the audited irreducible core, pinned with a written statement that any further decrease requires a new decision; the non-adapter visit-count baselines become the four documented structural sets; and each artifact-role exemption is re-examined against the new stances, since a role a language declares `unsupported` is a declared absence the gate reads rather than an exemption. Decision 28 invariant 4 governs every one: a count moves only in the same change as the work that moved it, and no gate is turned off to get there.
...
| B-1 | Every universe domain maps to a typed context model; the fingerprint context has zero consumers | Registry test; negative scan landed as a test |
| B-2 | Zero private per-language context builders remain; the only surviving per-package definitions are pure pre-binding adapters | Duplicate-body scan (Decision 34 invariant 1's shape) extended with this symbol family |
| B-3 | The language-axis drift baseline reaches the audited floor monotonically, one decrement per hoist | Drift gate; classification entry count equals live drifted count at every step |
| B-4 | Every hoist is byte-identical on the foundation test set for every affected language | Byte-identity generation, exit 0, output path recorded |
...
| C-4 | A row for a builtin in an `unsupported` group fails at registration | Unit test in `datrix-codegen-common` with the testkit fixture plugin |
