...
**Status:** Landed. `block-realization-parity-gate.ps1` (D1) passes with zero unexempted gaps across all 4 registered platforms (aws/azure/docker/local); `standing-conformance-gate.ps1` (D10) passes all 8 committed specs. See `datrix/scripts/test/quick-reference.md` for the full gate roster and mechanics.
...
- **D10 — The drift measurement is replaced with one that does not go quiet as drift sets in.** A name-keyed report over the registered language packages joins the repo-level validation scripts, deriving its target set from the entry-point registry at runtime rather than a hardcoded set, and refusing to pass with fewer than two targets. It is a **report with a decrease-only count baseline, not a pass/fail gate on individual names**: a name-keyed check cannot distinguish an intentional per-language emission difference from an unreconciled fix, and a gate that cannot make that distinction gets turned off. Every drifted group is classified once — intentional adaptation with a written reason, or a divergence to fix — with zero unclassified as the bar. A symbol carrying one registered language's name inside another language's package is a target-name leak the shared-layer ratchet cannot see, because that ratchet is scoped to the shared codegen package; such symbols are renamed.
...
| 10 | Parallel implementations across language packages are measured by a signal that survives divergence | Name-keyed report (`parallel-implementation-drift-gate.ps1`) with a runtime-derived target set, a decrease-only count baseline, and zero unclassified groups |
