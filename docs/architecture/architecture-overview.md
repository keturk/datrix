...
**Status:** Approved — implementation in progress. The instrument (scanner, gate, `name_tokens`, own-target-name ratchet, discovery-symbol relocation, old-scanner deletion) is the first increment; the identical-body hoists, same-decision hoists, CQRS port-then-hoist, and the remaining reconciliations follow in that order, each proven by regenerated output.
