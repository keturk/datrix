...
17. **A declared knob must be realized (Approved — Implementation In Progress)** -- A configuration field the system accepts is a promise it keeps: perturbing the field must change the emitted artifact in a functional position, not just pass load-time validation or land in a generated comment/docstring/log line. Every consumed field is realization-proven by an executable perturb-and-diff check or carries a reviewed, written exemption; a target that genuinely cannot realize a portable field declares it unsupported with a reason instead of silently no-op'ing. Enforced by per-package conformance checks that perturb a field, regenerate, and diff the output — these do not exist in the tree yet.
...
- **Purposeful mini-DSLs, closed and driving** -- genDSL/ConfigDSL/SeedDSL/RealizationDSL/EmitDSL are single-concern surfaces whose declarations are the *only* execution/emission path for the concern they own (never mere description, never a parallel imperative bypass), reject unknown references at load time, and default to typed data over text
