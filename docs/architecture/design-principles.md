...
**Example (rate limit preconditions):** Both rate limiting templates (`middleware_rate_limit.py.j2` for per-route and `rate_limit_dependency.py.j2` for plan-based) raise `HTTPException` **503** when `app.state.redis` is missing, and **429** when the limit is exceeded. There is no runtime bypass switch in the service: the only rate-limit bypass lives in the emitted deploy-test harness (`project/_deploy_test_main.py.j2`), never in service code.
...
**Example (runtime configuration):** Every deployment-static value a generated service needs is an explicit, named fact: on the python reference generator it is a baked `typing.Final` constant in `config/_bootstrap.py` or a typed `AppSettings` field assembled at startup from the config store and the secrets resolver by logical handle ([Decision 14](architecture-overview.md#decision-14-runtime-configuration--secrets--zero-environment-architecture-adopted)); on a language that declares the zero-environment contract unrealized it is a named environment variable that fails loud when unset. Nothing is an undocumented literal in generated service code, and nothing is silently defaulted.
...
### 11. A Declared Knob Must Be Realized (Adopted)
...
**F4 — Text is earned:** new surfaces default to typed data declarations with validating loaders; a textual grammar is justified only where authoring ergonomics demand it. genDSL's own tokenizer, parser, and validator measure 1,970 LOC — that cost buys a readable embedded-docstring surface for generator authors, not a default for every future decision family. Computation stays in Python: DSLs declare structure, never implement algorithms.

**F5 — Declarations are the only emission path (Adopted):** A parallel imperative path that emits the same artifacts a mini-DSL's declarations already describe is not a coexisting option — it is a bypass and a drift source. Once a concern has a declared row, the declared row is the sole emission path for it; a surviving parallel branch is closed, not tolerated. Held by the shared `emission_path` testkit gate (`datrix_codegen_common.testkit.gates.emission_path`), wired per language against each package's real compiled genDSL definitions (e.g. `datrix-codegen-python/tests/gendsl/test_python_satisfies_the_shared_emission_path_gate.py`); by the closed-compilation gate (`datrix_codegen_common.testkit.gates.closed_compilation`); and by the deletion of the two parallel-path instances this doctrine named — the orphaned `*_domain()` functions (`DOMAIN_FUNCTIONS` now holds only `pilot_domain`/`migration_domain`, pinned by `datrix-codegen-common/tests/unit/domains/test_shared_domain_library.py::TestDomainFunctionsRegistry`) and `SeedGeneratorHooks`/`DefaultSeedHooks`, deleted with no surviving consumer in any package's source or tests.

**F6 — Typed predicate columns, never text; plan modules for pipelines (Adopted):** Where a declaration schema cannot yet express a routing decision (receiver shape, arity, a literal argument, a transpiler-flag guard), the fix is to extend the schema with typed predicate columns — the authoring unit stays a table row, never a fallback to free text, and the branch is never left imperative indefinitely. Boundary: this is for single-step dispatch, not multi-step lowering. A pipeline that lowers a construct across several dependent steps (a query chain, a statement sequence) is not row dispatch and is never forced into one; its consolidation path is shared **plan** modules that hold the decision logic once. Template bodies are never shared across languages this way — decision logic and structure are what get shared, never the body. Held by EmitDSL's validating constructor (`emit_decl` / `EmitTable.__init__` in `datrix_codegen_common.transpiler.emit_dsl`, which fails loud at construction on an unmapped builtin, dangling emit-function reference, unknown chain step, unknown flag guard, or unknown receiver-type restriction) and by the typed predicate rows it validates today — the `Arity`-guarded `instance_call` rows and the `builtin_category_preference` table in `datrix-codegen-python/src/datrix_codegen_python/transpiler/emit_tables.py` — proven against real, unmutated tables by `datrix-codegen-python/tests/unit/transpiler/test_emit_dsl_closed_compilation.py` (`test_all_three_real_unmutated_tables_are_green`).

**F7 — Declaration and its check share one source (Adopted):** A declared surface's provisioning/output patterns (which artifacts a declaration is expected to produce) live on the declaration itself, never re-supplied as a parameter by whichever check polices it — so the declaration and the conformance check that verifies it cannot drift apart from each other. Held by the realization-conformance gate (`datrix_codegen_common.testkit.gates.realization_conformance`), which reads each `BlockRealization` cell's provisioning-artifact glob directly off `realization.structural_pattern` rather than a separately supplied mapping, exercised against real plugins by e.g. `datrix-codegen-docker/tests/unit/testkit/test_kit_ci.py` (`test_docker_block_realizations_self_consistent`).

**D1 completion bar.** A mini-DSL surface is COMPLETE when: every decision it owns is a declared row/cell rejected at load time if unknown; no parallel imperative site emits what the declaration describes; the residual per-target surface that is not declarable is documented with a per-item structural reason and pinned by a ratchet wired into the owning package's own suite; and the architecture docs describe the tree as it is. The bar is **deliberately not zero**: the repo has audited and rejected the zero target twice, in code. `datrix_codegen_common.testkit.gates.visit_adapter_ratchet` (`visit_adapter_ratchet.py:3-18`) states it in its own words: *"This is a MONOTONIC ratchet over a documented, justified NON-ZERO terminal floor — not a countdown to zero. The originally intended check … is NOT reachable: a live-tree audit (2026-07-11) found that a structurally-necessary set of transpiler functions is genuinely language-owned."* The transpiler skeleton subpackage (`datrix_codegen_common.transpiler.skeleton`, `__init__.py:3-13`) records the same finding from the other direction: *"An early assumption held that ~35 'structurally identical' visitor / dispatch function pairs existed … A live-tree readiness audit (2026-07-11) disproved that premise."*
