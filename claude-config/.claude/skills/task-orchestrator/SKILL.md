...
- **Sequential multi-phase execution, with no stop in between** — given several phases (e.g. `72, 73, 74`), finishes each phase fully and then **immediately starts the next, unprompted**. At every phase boundary a gate (Step 3i) runs `affected-gate.ps1` over its **sweep set** (the run's cumulative changed packages + their reverse-dependency closure per `d:\datrix\.claude\skills\_shared\verification-strategy.md`; the gate carries every package whose inputs are unchanged since its last green full run, so the sweep is ALL packages only when the closure is everything, e.g. a `datrix-common` change) and fixes **all** failures — including pre-existing ones unrelated to the phase's changes — with Opus-led recovery. Green gate → next phase's first wave dispatches in the same turn; the run ends only at the last phase (or a Step C halt). See **Multi-Phase Continuation**
...
- `package_change_log{}` — map of `package → every file changed in that package this run and by whom` (task agents' `files_created`/`files_modified` + `scope_expansion`, fix subagents' reported changes, audit-authored task files). Fed by 3c/3e/3i; consumed by the 3i sweep-set determination (which packages changed). It records what changed; it never decides what may skip a re-run — `affected-gate.ps1`'s own suite-input fingerprint decides which sweep-set packages are unchanged since their last green full run, and its `CARRIED` rows are that record
...
**Quality-gate tasks — suppress the agent's full-suite run AND its design-conformance scan.** A `**Category:** Quality Gate` task file lists "Write the full-suite ticket and run the gate (`affected-gate.ps1 -Projects {package-name}`)" as a verification step (its `## Targeted Tests` scope is the full suite) and carries a design-conformance scan (checklist item 2b). In an orchestrated run BOTH are owned by the orchestrator — the full suite by the phase-boundary sweep (3i Step A) and the design-conformance sweep by the phase-boundary conformance gate (3i Step A2) — so the agent running either is a pure duplicate. When spawning a quality-gate agent, append this directive to its prompt:

> Do NOT run the full test suite (the task's `affected-gate.ps1 -Projects {package-name}` step, or any whole-package `test.ps1` form) — skip Verification Step 1 / the `## Targeted Tests` gate command; the orchestrator's phase-boundary sweep owns that verdict, and `guard-full-suite-runs.py` blocks a subagent from running it. Do NOT execute the design-conformance scan (checklist item 2b — per-invariant surface sweeps and acceptance checks); the orchestrator's phase-boundary conformance gate owns those executions. Run no tests at all. Perform the remaining static verification in full: the non-trivial-implementation scan (stubs / `TODO` / `pass` / `NotImplementedError`, always-true validators, legacy/dual paths), the coverage & test-quality sanity checks (read the tests; do not run them), and the "How Solved" self-contradiction scan. Report every finding in your JSON result; the orchestrator owns the pass/fail verdict.
...
1. **Determine the sweep set.** Build it from `package_change_log{}` (which includes task agents' `files_created`/`files_modified`, every `scope_expansion`, and every fix-subagent edit from 3e/3f — not just the task files' declared lists):
   ...
   - **Multi-phase runs — the affected-green guarantee, computed by the gate itself:** at every phase boundary, the sweep set is the CUMULATIVE affected set (changed + reverse-dependency closure) of everything the run has touched so far — never unconditionally all packages, and never hand-trimmed; a package outside the closure cannot have been broken by this run, and pre-existing rot in untouched packages is the scheduled full sweep's job, not this gate's. Pass that whole cumulative set to the gate in step 2. The gate's own suite-input fingerprint (the package trees in the cone, the installed distributions, the interpreter, and the foreign inputs the last run observed) decides, package by package, whether a re-run is needed: a package whose newest green full run is under 24 h old and whose fingerprint still matches comes back `CARRIED` with no suite launched — green-at-last-run plus byte-identical-inputs-since IS "all tests passing" for that package. The `CARRIED` rows (`run_dir`, `age_minutes`, `fingerprint` in the gate's `Details:` JSON) are the record of what stood in; there is no hand-kept ledger and no safety valve to reason about, because the fingerprint sees every change on disk, whether or not an agent report attributed it.
   - **Repo gates:** add repo gates only when their surface was touched (per `_shared/verification-strategy.md` "Repo gates"). There is no stored-baseline output gate to add: an output-preservation claim is proven by tests in the owning package, and `artifact-role-parity-gate.ps1` runs only over a complete local `.generated/` corpus, which Jon generates.
   - **Single-phase runs:** same rule — changed + reverse-dependency closure.
2. **Write the full-suite ticket, then run the sweep set concurrently and read the verdict in one call.** `guard-full-suite-runs.py` blocks `affected-gate.ps1` in the main session unless `D:\datrix\.tmp\full-suite-ticket.json` covers every package you pass to `-Projects`. Write it with the Write tool: `{"packages": ["{pkg1}", "{pkg2}", ...], "reason": "phase {P} boundary gate, sweep set = cumulative affected set", "granted_by": "orchestrator", "expires_epoch": <now + at most 6h, integer epoch seconds>}` (the reason must be at least 10 characters). Then:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/affected-gate.ps1" -Projects {pkg1},{pkg2},...
   ```
   Include `VERIFIED_AGAINST_QUICK_REFERENCE` in the Bash tool description. `affected-gate.ps1` re-derives the closure of every package you pass (so it never runs a narrower set than step 1 computed), carries each package whose fingerprint still matches its newest green full run (verdict `CARRIED`, no child launched, pinned to that run), launches one `test.ps1` child per remaining package concurrently under a worker budget, then aggregates one GREEN/RED verdict by reusing `gate-verdict.ps1`'s own per-project evaluation — this replaces firing `test.ps1` per package and separately calling `gate-verdict.ps1`. It prints one line per package (`CARRIED run=…` or `running -- <reason>` before launch; GREEN/RED/CARRIED after) + `OVERALL`, writes per-package counts and capped failing-test lists to its `Details:` JSON, and appends one completion row (`ran`, `carried`, `overall`) to `D:\datrix\.tmp\full-suite-audit.jsonl`. Sanity-check each package's `run_dir`/`age_minutes` in the JSON: a ran package's run is the one its own child produced; a `CARRIED` package's is the earlier green run that stood in. A package is GREEN only when `result == "PASSED"` AND `counts.failed == 0` AND `counts.error == 0` — errors count as red, exactly as the 3d gate rules; the script already applies this (UNKNOWN/in-progress/missing results are RED), and `CARRIED` counts as green. The gate's own `carried`/`run_dir`/`fingerprint` fields in its `Details:` JSON ARE the record — no separate bookkeeping is needed. For each RED package, run `collect-failure-data.ps1` on its run dir for the cluster detail that drives step 3.
3. **Fix every red package to GREEN — regardless of attribution.** For each failing test and each erroring module across ALL packages in the sweep set, **including failures in code that no task in this phase modified**:
   ...
   - Verify authoritatively: re-run the specific test (`test.ps1 {package} -Specific "{path}"`), then re-run `affected-gate.ps1 -Projects {package}` (step 4) — reading its `Details:` JSON, not the agent's self-report. The fix subagent runs only its targeted tests and reports the packages it changed; it never runs the gate.
...
4. **Re-run until clean** — after fixes, update the ticket if the package list grew, and re-run `affected-gate.ps1 -Projects {red packages}` for the fresh verdict (their green peers' recorded results stand; the gate re-derives the red packages' closure and carries any member whose inputs did not change). Repeat until every sweep-set package is GREEN, or escalation/halt is reached. An error fixed in one module often unhides many tests that never ran, so always re-run the failing package through the gate after a fix rather than trusting `-Specific` alone. If a fix touched a package OUTSIDE the current red set (scope expansion into a green or unswept package), add that package to the sweep set and to the next gate call's `-Projects` list.
...
Any `failed` or `skipped` task in phase `P`, **OR** any package still red after Step A's fix loop, **OR any unresolved design-conformance failure from Step A2** (an unenforced design-named surface, an unproven task acceptance property, or an open conformance gap):
   ...
   - **Dispatch subagents to implement your recovery plan** (**sonnet**, or **opus** for the hardest items), partitioned so no two agents write the same files: give each the per-item remediation steps, exact files to modify, and the CLAUDE.md constraints. The implementers run only their targeted tests and report the packages they changed. Then update the ticket and re-run `affected-gate.ps1 -Projects {every affected package}` yourself (3i Step A rules — GREEN only when `result == "PASSED"` AND `failed == 0` AND `error == 0`; `CARRIED` counts as green). Re-attribute and mark any now-passing tasks complete via `complete.ps1`.
...
(`carried green` names the packages `affected-gate.ps1` itself reported `CARRIED` at this boundary's sweep — read directly from its `Details:` JSON, never hand-tracked. Multi-phase runs only; omit for single-phase runs.)
...
CONSTRAINTS: NO workarounds / band-aids / xfail-to-pass / conditional guards that hide the broken path; NO git reverts; NO mocks; NO debug scatter; run only your targeted tests, via test.ps1 -Specific or test-single.ps1 (never pytest directly, never a whole suite or the gate — report the packages you changed and I run the gate); no -NoSave/-VerboseOutput.
...
- For a phase-recovery plan: after the implementers return, update the ticket and re-run `affected-gate.ps1 -Projects {every affected package}` yourself (3i Step A rules — gate-derived closure, `CARRIED` rows for anything unchanged) before re-evaluating the phase gate.
