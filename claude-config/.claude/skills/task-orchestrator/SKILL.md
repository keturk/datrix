...
**The orchestrator's mandate is conformance, not throughput** (CLAUDE.md "Task Orchestration" states the full rationale — it binds here). Two consequences at every wave and phase boundary: **green targeted tests are necessary but NOT sufficient** (the explicit design-conformance gates at 3g and 3i Step A2 prove the design invariant itself), and **a task whose agent returned BLOCKED can never be quietly marked COMPLETED** (per the shared checklist `d:\datrix\.claude\skills\_shared\completion-eligibility.md`).
...
- **Sequential multi-phase execution, with no stop in between** — given several phases (e.g. `72, 73, 74`), finishes each phase fully and then **immediately starts the next, unprompted**. At every phase boundary a gate (Step 3i) re-runs the **union of the phase's targeted tests** (every task's `## Targeted Tests` files and feature tags, per package — never a whole suite) and the design-conformance gate, and fixes **all** failures they show with Opus-led recovery. Green gate → next phase's first wave dispatches in the same turn; the run ends only at the last phase (or a Step C halt). See **Multi-Phase Continuation**
...
**Run this before ANY execution.** The task set was authored against the design and the codebase **as they were when `/generate-tasks` ran**; both may have moved since, and the generator may have missed a surface. Executing an insufficient task set produces the same failure mode every time — every task COMPLETED, the tests green, and the design still unenforced. The audit answers one question: *if every task in this set succeeds exactly as written, will the `design_contract` from 1d hold over the code that is actually on disk today?* If the answer is anything but yes, the audit **adds the missing tasks and rewires dependencies** before Step 2 builds the DAG.
...
5. **O5 Repair a defective `## Targeted Tests`.** A task naming a whole-suite run (a `test.ps1` run with a package and none of `-Specific`/`-Keyword`/`-Tag`, `-All`, `-Rerun`, a tier sweep, or `affected-gate.ps1`) is a defective task file per CLAUDE.md, and `guard-full-suite-runs.py` blocks every agent from running it — so leaving it is a dispatch that will fail. Replace it with the specific test files and the feature tags (per package) covering the code that task changes.
...
Tests:    task-{NN}-{TT} — whole-suite run → {specific files} + tags {tags}
```
...
**Multi-phase runs (phase ordering is a hard constraint):** run `plan-waves.ps1` once per phase, in numeric order, and concatenate the wave lists — ALL of an earlier phase's waves complete before ANY later phase's first wave. Each phase boundary is also a **Phase Boundary Gate** (Step 3i): the earlier phase must pass an explicit completion check — the union of the phase's targeted tests (every task's `## Targeted Tests` files and feature tags, per package) re-run once and green, and the design-conformance gate passed, with Opus-led recovery on failure — before the next phase's first wave is spawned. No whole suite runs at any boundary.
...
Once waves are assigned, the 3d test gate runs **targeted tests only** for every wave of the phase, and the phase-boundary gate (3i) re-runs the phase's targeted tests once. **No whole suite runs anywhere in this skill** — not in a wave, not at a phase boundary, not in a Quality Gate task, for any package, for any reason (`guard-full-suite-runs.py` refuses every form).
...
**Multi-phase runs carry a TARGETED-TESTS-GREEN guarantee.** At every phase boundary the orchestrator must be able to assert "every targeted test of every change this phase made — its files and the feature tags of every changed behaviour, in every package the change reaches — passes", established by 3i Step A's one targeted re-run. Whole suites are never run by the orchestrator; Jon runs them himself. This guarantee is what lets phase `P+1` build on `P` without inheriting breakage the run itself created. It never creates a stopping point: a red test found at the boundary is **fixed at the gate** (Step A's attribution-agnostic fix loop), and a green gate flows directly into the next phase's first wave in the same turn.

**At a green phase boundary you do not stop, do not summarize-and-yield, and do not ask.** Passing 3i's gates (Step A targeted re-run green + Step A2 design-conformance) is the *permission* to continue, not a milestone to report and rest on. The Phase Checkpoint is a one-line progress marker emitted **on the way into** the next phase's first wave — in the same turn, with no intervening question to the user. If you catch yourself writing a summary of what phase `P` accomplished while phase `P+1` still has unexecuted waves, you have stopped early: dispatch the next wave instead.
...
**Quality-gate tasks — static review only; no test runs, no conformance scan.** A `**Category:** Quality Gate` task is a static review of the phase's work: it runs no test suite (no agent ever does), and its design-conformance scan (checklist item 2b) is owned in an orchestrated run by the phase-boundary conformance gate (3i Step A2). The phase's targeted tests are re-run by 3i Step A. When spawning a quality-gate agent, append this directive to its prompt:

> Run no tests at all — never a whole suite (`guard-full-suite-runs.py` refuses every form), and not the phase's targeted tests either: the orchestrator's phase-boundary gate re-runs those once. Do NOT execute the design-conformance scan (checklist item 2b — per-invariant surface sweeps and acceptance checks); the orchestrator's phase-boundary conformance gate owns those executions. Perform the static verification in full: the non-trivial-implementation scan (stubs / `TODO` / `pass` / `NotImplementedError`, always-true validators, legacy/dual paths), the coverage & test-quality sanity checks (read the tests; do not run them — including: every test the phase added carries a feature tag, and every changed behaviour has a tagged test), and the "How Solved" self-contradiction scan. Report every finding in your JSON result; the orchestrator owns the pass/fail verdict.

This keeps the gate's independent static-analysis value (a different agent than the implementers reads everything) while each execution — targeted re-run and conformance — happens exactly once, at the phase boundary. The QG agent's findings feed 3i Step A2 as input (A2 must read them and act on each one); they are not a substitute for A2's own executions.
...
#### 3d. Run the Wave Test Gate Per Package (targeted ONLY — never a whole suite)
...
**HARD RULE — NO WHOLE SUITE, ANYWHERE. THE LIST OF EXCEPTIONS IS EMPTY.** You run **only the targeted tests of what the wave actually changed**: files (`-Specific`) and feature tags (`-Tag`). There is no last-touch-wave exception, no "the package is done so gate it now" exception, no phase-boundary sweep, and — read this twice — **no shared-layer exception.** `guard-full-suite-runs.py` refuses every whole-suite form for you as for every agent.

At the wave join, the gate covers — for each `package` with completed tasks in this wave — **only** the union of the `## Targeted Tests` files and tags from that package's completed tasks in this wave, plus the specific test files covering any code the wave moved, changed, or deleted. Coverage does NOT mean re-executing all of it: the gate below verifies each agent's already-saved run artifacts first and re-executes only what artifacts cannot prove.

**Shared-layer changes.** CLAUDE.md's cross-surface impact rule says a change to a shared layer (`datrix-common`, `datrix-codegen-common`, any shared contract) must not break any consuming package. Cover it with the **feature tags of the changed behaviour, run in every package the change reaches** (`test.ps1 {pkg-a} {pkg-b} -Tag {tag}` — reach established per `_shared/verification-strategy.md`), plus the specific test files exercising the moved/changed symbol. That is the whole proof; no sweep follows it.

After the **wave join** — the rolling pool has fully drained (`in_flight` and `wave_queue` both empty):

1. Group completed tasks in this wave by `package`, and assemble each package's **wave targeted set**: the union of its tasks' `## Targeted Tests` files and tags + the specific test files covering code the wave moved, changed, or deleted.
...
3. **Re-run — batched — only what artifacts cannot prove.** For each affected package, ONE `test.ps1` invocation with a comma-separated `-Specific` list (multi-path batching runs the whole set in a single pytest session — never one invocation per file), and ONE `-Tag` invocation over the packages that carry the wave's tags, covering only:
   ...
   ```bash
   # ALWAYS targeted. A bare `test.ps1 {package-name}` (no -Specific/-Keyword/-Tag)
   # is a WHOLE SUITE and is FORBIDDEN — everywhere, for every agent.
   powershell -File "d:/datrix/datrix/scripts/test/test.ps1" {package-name} -Specific "{test-path-1},{test-path-2},{test-path-3}"
   powershell -File "d:/datrix/datrix/scripts/test/test.ps1" {package-a} {package-b} -Tag {tag-1},{tag-2}
   ```

   Run the re-runs one `test.ps1` invocation at a time — concurrent invocations contend for the shared venv's package lock and the loser dies without running anything; one invocation already covers several packages. Include `VERIFIED_AGAINST_QUICK_REFERENCE` in each Bash tool description. If nothing in a package falls under (a)–(d), that package's gate rests entirely on its accepted artifacts — that is the intended outcome, not a skipped gate.
...
For each failing test **and each erroring module** from the wave gate (always targeted — no gate ever runs a whole suite):
...
After the fix loop, re-run the gate once to verify no regressions: the package's **wave targeted set** (files and tags), batched per package. If a fix modified code **outside** the wave tasks' own files, never escalate to a whole suite — **widen the targeted re-run** to include the specific test files covering the fix's added files and the feature tags of the behaviour the fix changed (for a shared-layer file, run those tags in every package the fix reaches), and record those files and tags in `package_change_log{}` so the 3i re-run picks them up at the boundary.
...
##### Step A — Phase-end targeted re-run (fix ALL failures, attribution-agnostic)

Stricter than 3d: **every test in the phase's targeted set must pass with zero failures and zero errors — pre-existing failures included** (3d's pre-existing allowance does NOT hold here; they are blocking and must be driven to zero before phase `P+1`). Runs every time a phase completes, even a fully green one — a later wave can break an earlier wave's tests, and only a re-run against the complete phase shows it. **It is a targeted re-run, never a whole suite.**

1. **Assemble the phase's targeted set.** From every task in phase `P` and every fix recorded in `package_change_log{}` (task agents' `files_created`/`files_modified`, every `scope_expansion`, every fix-subagent edit from 3e/3f): per package, the union of the `## Targeted Tests` files, the test files covering changed code, and the feature tags of every changed behaviour. A behaviour changed in a shared layer carries its tags into **every package the change reaches** (per "Which packages a change reaches" in `d:\datrix\.claude\skills\_shared\verification-strategy.md`) — a package the change reaches is in the set *even though no task modified it*. When you cannot tell whether a package is reached, read its tests and conftest until you can. Add repo gates only when their surface was touched (per `_shared/verification-strategy.md` "Repo gates").
2. **Run it, batched, and read the verdict from `index.json`.** One `test.ps1` call per form, sequentially (concurrent invocations contend for the shared venv's package lock):
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test.ps1" {pkg1} -Specific "{path-1},{path-2}"
   powershell -File "d:/datrix/datrix/scripts/test/test.ps1" {pkg1} {pkg2} {pkg3} -Tag {tag-1},{tag-2}
   ```
   Include `VERIFIED_AGAINST_QUICK_REFERENCE` in the Bash tool description. A run is GREEN only when `result == "PASSED"` AND `counts.failed == 0` AND `counts.error == 0` — errors count as red, exactly as the 3d gate rules. For each RED run, run `collect-failure-data.ps1` on its printed run folder for the cluster detail that drives step 3.
3. **Fix every red test to GREEN — regardless of attribution.** For each failing test and each erroring module in the targeted set, **including failures in code that no task in this phase modified**:
   ...
   - Verify authoritatively: re-run the specific test (`test.ps1 {package} -Specific "{path}"`), then the targeted set of the fixed package and the tags of the behaviour the fix changed in every package it reaches (step 4) — reading `index.json`, not the agent's self-report. The fix subagent runs only its targeted tests and reports the packages, surface and tags it changed.
...
4. **Re-run until clean** — after fixes, re-run the red runs' targeted forms plus the files and tags of whatever the fixes changed, in every package the fixes reach. Repeat until the phase's whole targeted set is GREEN, or escalation/halt is reached. An error fixed in one module often unhides many tests that never ran, so re-run the module's whole targeted form after a fix, not just the one node id. If a fix touched a package OUTSIDE the current set, add its tests and tags to the set.

##### Step A2 — Phase-end DESIGN-CONFORMANCE gate (runs at EVERY phase end, including single-phase runs)

Green tests prove the code runs; they do NOT prove the design holds. This gate verifies phase `P` actually satisfied the `design_contract` built in Step 1d. **It runs at the end of every phase — including a single-phase run — and a phase cannot be declared complete without it, even when Step A is fully green.** Skipping it is exactly how a half-enforced invariant has shipped undetected before.
...
2. **Any surface in the design's set that is unguarded / unconverted is a CONFORMANCE FAILURE** — even if every task is COMPLETED and every test run is GREEN. A half-implemented invariant (guarded on the easy surface, silently dropped on the rest) is exactly the escape mode this step is built to catch.
...
**Green phase** — the phase's whole targeted set passed Step A (all red driven to zero) **AND Step A2's design-conformance gate passed (every design-named surface enforced, every task's acceptance property proven, no open conformance gap)** AND `failed` and `skipped` are both empty **AND every task in the phase is COMPLETE — including any task you or an agent filed mid-run**:
...
Any `failed` or `skipped` task in phase `P`, **OR** any targeted run still red after Step A's fix loop, **OR any unresolved design-conformance failure from Step A2** (an unenforced design-named surface, an unproven task acceptance property, or an open conformance gap):
   ...
   - **Dispatch subagents to implement your recovery plan** (**sonnet**, or **opus** for the hardest items), partitioned so no two agents write the same files: give each the per-item remediation steps, exact files to modify, and the CLAUDE.md constraints. The implementers run only their targeted tests and report the packages, surface and tags they changed. Then re-run the phase's targeted set plus the tests and tags of what the recovery changed yourself (3i Step A rules — GREEN only when `result == "PASSED"` AND `failed == 0` AND `error == 0`; never a whole suite). Re-attribute and mark any now-passing tasks complete via `complete.ps1`.
...
```
Phase {P} COMPLETE — {completed}/{phase_total} tasks | targeted tests: {package}: {passed}/{total} | {package}: {passed}/{total} | tags: {tag, tag}
→ Starting phase {P+1}
```
...
3. **Decide, per doc:**
   - **Every referencing task COMPLETED**, and every phase this run executed for that doc passed 3i Step A (targeted re-run) **and** Step A2 (design-conformance) → update the status, per item 4 below.
...
4. **Rewrite the `Status:` line and only that line.** Use `Edit` with the doc's existing status line as the exact `old_string`, so the change cannot spill past it:
   ```
   Status: **Implemented — phase(s) {NN}[, {NN}] complete; verified by the phase-boundary targeted-test and design-conformance gates.**
   ```
...
- **CONFORMANCE OVER THROUGHPUT** — enforced by the 3g completion checklist (`_shared/completion-eligibility.md`) and the 3i Step A2 conformance gate; never relaxed for green tests.
...
CONSTRAINTS: NO workarounds / band-aids / xfail-to-pass / conditional guards that hide the broken path; NO git reverts; NO mocks; NO debug scatter; run only your targeted tests, via test.ps1 -Specific / -Tag or test-single.ps1 (never pytest directly, never a whole suite — report the packages, surface and tags you changed); no -NoSave/-VerboseOutput.
...
- For a phase-recovery plan: after the implementers return, re-run the phase's targeted set plus the files and tags of what the recovery changed, in every package it reaches, yourself (3i Step A rules — never a whole suite) before re-evaluating the phase gate.
