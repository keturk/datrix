...
Diagnose and fix failures, errors, and warnings in `datrix-language` from a structured test results `index.json`, trace to root cause in the DSL parser and language package source code, fix, and verify — without needing to explore the codebase first.
...
- **Package:** `datrix-language` — DSL parser and language package — source at `d:\datrix\datrix-language\`
...
### Project Structure
Read `d:\datrix\datrix-language\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-language`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-language -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-language
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-language -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-language -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-language -VerboseOutput
   ```
...
- About to modify code **outside datrix-language**
