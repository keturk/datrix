...
- **Constraints** — the CLAUDE.md rules that bite for this packet (no workarounds, no git reverts, full type hints but never run a type-checker, no mocks, domain isolation, temp files only under `D:\datrix\.tmp\`/`.scripts\`/`.test-output\`).
