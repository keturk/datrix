...
Diagnose and fix failures, errors, and warnings in `datrix-extensions` from a structured test results `index.json`, trace to root cause in the extensions package source code, fix, and verify — without needing to explore the codebase first.
...
- **Package:** `datrix-extensions` — extensions package — source at `d:\datrix\datrix-extensions\`
...
### Project Structure
Read `d:\datrix\datrix-extensions\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-extensions`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-extensions -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-extensions
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-extensions -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-extensions -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-extensions -VerboseOutput
   ```
...
- About to modify code **outside datrix-extensions**
