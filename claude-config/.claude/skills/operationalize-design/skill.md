...
**HARD CONSTRAINT — Task Location Allowlist:** the full allowlist (17 framework projects) and its rules live in `/generate-tasks` SKILL.md ("Task Location Allowlist — HARD CONSTRAINT"), which the mandatory first step below makes you read. The bindings that bite here: every task file path AND `dependencies.md` MUST begin with `D:\datrix\{project}\.tasks\` where `{project}` is one of the 17 allowlisted names; **the design document being operationalized often lives inside a customer/generated project (e.g. `D:\<Product>\`, or one of its sibling repos when that product is split across several) — the tasks it produces NEVER go there**; tasks with no specific package go in the `D:\datrix\datrix\.tasks` fallback. Task tooling only scans `D:\datrix\*/.tasks` — a `.tasks` folder anywhere else is invisible and its tasks silently never run.
...
4. Generate task files following the **exact template format** from `/generate-tasks` SKILL.md (lines 127-200+):
   ...
   **File naming:**
   ...
   ```
   `{repo}` MUST be one of the 17 framework projects in the **Task Location Allowlist** above — never a customer/generated project, even when the design doc lives in one. Use `datrix` as the fallback bucket when no specific package fits.
...
- [ ] **Every task file path and `dependencies.md` begins with `D:\datrix\{project}\.tasks\` where `{project}` is one of the 17 in the Task Location Allowlist** — no `.tasks` folder under a customer/generated project or any path outside the allowlist; tasks with no specific package use the `datrix\.tasks` fallback
