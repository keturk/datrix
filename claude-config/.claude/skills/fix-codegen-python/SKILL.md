...
Diagnose and fix failures, errors, and warnings in `datrix-codegen-python` from a structured test results `index.json`, trace to root cause in Python codegen generator/template/transpiler code, fix, and verify — without needing to explore the codebase first.
...
- **Language:** Python codegen ONLY. Do NOT cross into TypeScript codegen packages.
- **Package:** `datrix-codegen-python` — Python codegen — source at `d:\datrix\datrix-codegen-python\`
...
### Project Structure
Read `d:\datrix\datrix-codegen-python\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-codegen-python`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-codegen-python -VerboseOutput
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-python -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-codegen-python -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-codegen-python -VerboseOutput
   ```
...
- About to modify code **outside datrix-codegen-python**
