---
description: Diagnose and fix datrix-codegen-component test failures, errors, and warnings from structured test results
model: sonnet
---

# Fix Codegen Component

Diagnose and fix failures, errors, and warnings in `datrix-codegen-component` from a structured test results `index.json`, trace to root cause in component codegen source code, fix, and verify — without needing to explore the codebase first.
...
```
/fix-codegen-component D:\datrix\datrix-codegen-component\.test_results\test-results-YYYYMMDD-HHMMSS\index.json
```
...
- **Package:** `datrix-codegen-component` — component codegen — source at `d:\datrix\datrix-codegen-component\`
...
{
  "schema_version": 1,
  "project": "datrix-codegen-component",
...
### Project Structure
Read `d:\datrix\datrix-codegen-component\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-codegen-component`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-codegen-component -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-component
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-component -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-codegen-component -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-codegen-component -VerboseOutput
   ```
...
```
FIX-CODEGEN-COMPONENT COMPLETE
...
- About to modify code **outside datrix-codegen-component**
