...
Diagnose and fix failures, errors, and warnings in `datrix-codegen-docker` from a structured test results `index.json`, trace to root cause in Docker infrastructure codegen source code, fix, and verify — without needing to explore the codebase first.
...
- **Package:** `datrix-codegen-docker` — Docker infrastructure codegen — source at `d:\datrix\datrix-codegen-docker\`
...
### Project Structure
Read `d:\datrix\datrix-codegen-docker\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-codegen-docker`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-codegen-docker -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-docker
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-docker -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-codegen-docker -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-codegen-docker -VerboseOutput
   ```
...
- About to modify code **outside datrix-codegen-docker**
