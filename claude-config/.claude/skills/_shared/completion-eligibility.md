...
**A follow-up task goes in the phase you are executing — never a new one.** Put it in the owning package's existing `.tasks\phase-{NN}\`, numbered as that phase's next free `{TT}` (`validate-dependencies.ps1 -Phase {NN} -NextTaskNumber`). **No agent may create a `.tasks\phase-NN\` directory that does not already exist** — creating a phase is a planning act reserved to Jon and the planning skills he invokes by name. See CLAUDE.md "Task Orchestration" and execution-contract §5.

**Phase completion is the AND of its tasks.** A phase is COMPLETE only when every task in it — including any you filed mid-run — is COMPLETE, or carries a valid B1–B4 blocker with the four-part proof. A task filed during the phase is added to that phase's completion bar, not exempted from it; you finish it before declaring the phase done. Reporting a phase green while one of its own tasks sits NOT STARTED is a false completion, and filing a task forward to dodge a gate is the failure this rule exists to stop.
