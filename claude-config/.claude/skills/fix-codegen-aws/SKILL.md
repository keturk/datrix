---
description: Diagnose and fix datrix-codegen-aws test failures, errors, and warnings from structured test results
model: sonnet
---

# Fix Codegen AWS

Diagnose and fix failures, errors, and warnings in `datrix-codegen-aws` from a structured test results `index.json`, trace to root cause in generator/template source code, fix, and verify — without needing to explore the codebase first.
...
```
/fix-codegen-aws D:\datrix\datrix-codegen-aws\.test_results\test-results-YYYYMMDD-HHMMSS\index.json
```
...
- **Package:** `datrix-codegen-aws` — AWS infrastructure codegen — source at `d:\datrix\datrix-codegen-aws\`
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-codegen-aws -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-aws
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-aws -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-codegen-aws -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-codegen-aws -VerboseOutput
   ```
...
- About to modify code **outside datrix-codegen-aws**
