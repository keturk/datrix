...
Diagnose and fix failures, errors, and warnings in `datrix-codegen-aws` from a structured test-results `index.json`.
...
- `{PACKAGE}` = `datrix-codegen-aws`
- `{PACKAGE_PATH}` = `d:\datrix\datrix-codegen-aws\`

## Package Specifics

- **Scope:** `datrix-codegen-aws` — AWS infrastructure codegen.
