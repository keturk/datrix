...
Diagnose and fix failures, errors, and warnings in `datrix-cli` from a structured test results `index.json`, trace to root cause in source code, fix, and verify — without needing to explore the codebase first.
...
- **Package:** `datrix-cli` — CLI tool — source at `d:\datrix\datrix-cli\`
...
### Project Structure
Read `d:\datrix\datrix-cli\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-cli`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-cli -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-cli
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-cli -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-cli -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-cli -VerboseOutput
   ```
...
- About to modify code **outside datrix-cli**
