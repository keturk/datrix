...
Diagnose and fix failures, errors, and warnings in `datrix-common` from a structured test results `index.json`, trace to root cause in the shared framework library source code, fix, and verify — without needing to explore the codebase first.
...
- **Package:** `datrix-common` — shared framework library — source at `d:\datrix\datrix-common\`
...
- **CAUTION:** Changes to datrix-common can affect ALL downstream packages. Be especially careful with API changes.
...
### Project Structure
Read `d:\datrix\datrix-common\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-common`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-common -VerboseOutput
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-common -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-common -VerboseOutput
   ```
...
- About to modify code **outside datrix-common**
...
2. Activate the fix skill for that project by invoking `/fix-{suffix}` where the suffix drops the `datrix-` prefix (e.g., `datrix-codegen-typescript` → `/fix-codegen-typescript`, `datrix-language` → `/fix-language`).
