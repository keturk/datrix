---
description: Diagnose and fix datrix-codegen-sql test failures, errors, and warnings from structured test results
model: sonnet
---

# Fix Codegen SQL

Diagnose and fix failures, errors, and warnings in `datrix-codegen-sql` from a structured test results `index.json`, trace to root cause in SQL codegen source code, fix, and verify — without needing to explore the codebase first.
...
```
/fix-codegen-sql D:\datrix\datrix-codegen-sql\.test_results\test-results-YYYYMMDD-HHMMSS\index.json
```
...
- **Package:** `datrix-codegen-sql` — SQL codegen — source at `d:\datrix\datrix-codegen-sql\`
...
### Project Structure
Read `d:\datrix\datrix-codegen-sql\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-codegen-sql`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-codegen-sql -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-sql
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-sql -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-codegen-sql -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-codegen-sql -VerboseOutput
   ```
...
- About to modify code **outside datrix-codegen-sql**
