...
Diagnose and fix failures, errors, and warnings in `datrix-codegen-common` from a structured test results `index.json`, trace to root cause in shared codegen library source code, fix, and verify — without needing to explore the codebase first.
...
- **Package:** `datrix-codegen-common` — shared codegen library — source at `d:\datrix\datrix-codegen-common\`
...
### Project Structure
Read `d:\datrix\datrix-codegen-common\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-codegen-common`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-codegen-common -VerboseOutput
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-common -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-codegen-common -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-codegen-common -VerboseOutput
   ```
...
- About to modify code **outside datrix-codegen-common**
