...
Step 9's regression check applies unchanged: the originally failing and edited test files (`-Specific`), then the feature tags of the changed behaviour — `powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-vscode -Tag {tag-1},{tag-2}` (no package consumes the VS Code client). A Node test carries its tags as `#tag` tokens in its own or an enclosing `describe()` name; `-Tag` becomes Node's `--test-name-pattern`, and `-Tag` cannot be combined with `-Keyword` on a Node suite. `-ListTags` cannot enumerate a Node suite without running it — read the test names. Never a whole suite, never `-Fast`.
