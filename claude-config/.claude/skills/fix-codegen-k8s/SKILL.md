...
Diagnose and fix failures, errors, and warnings in `datrix-codegen-k8s` from a structured test results `index.json`, trace to root cause in Kubernetes infrastructure codegen source code, fix, and verify — without needing to explore the codebase first.
...
```
/fix-codegen-k8s D:\datrix\datrix-codegen-k8s\.test_results\test-results-YYYYMMDD-HHMMSS\index.json
```
...
- **Package:** `datrix-codegen-k8s` — Kubernetes infrastructure codegen — source at `d:\datrix\datrix-codegen-k8s\`
...
### Project Structure
Read `d:\datrix\datrix-codegen-k8s\.project-structure.md`. Regenerate if missing: `powershell -File "d:/datrix/datrix/scripts/dev/project-structure.ps1" datrix-codegen-k8s`.
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "tests/path/to/test_file.py::TestClass::test_method" -Project datrix-codegen-k8s -VerboseOutput
```

**Full suite (from bash):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-k8s
```

**Fast suite (skip slow tests):**
```bash
powershell -File "d:/datrix/datrix/scripts/test/test.ps1" datrix-codegen-k8s -Fast
```
...
```bash
powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}::{TestClass}::{test_method}" -Project datrix-codegen-k8s -VerboseOutput
```
...
2. **Verify warnings are resolved** by running the affected test file and checking output for remaining warnings:
   ```bash
   powershell -File "d:/datrix/datrix/scripts/test/test-single.ps1" "{test_file}" -Project datrix-codegen-k8s -VerboseOutput
   ```
...
- About to modify code **outside datrix-codegen-k8s**
