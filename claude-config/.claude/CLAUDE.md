...
**The Stop gate applies to `/task-orchestrator` and to nothing else.** `UserPromptSubmit` → `arm-orchestration-run.py` arms it only when Jon invokes that one skill (both forms: the typed `/task-orchestrator` and the expanded skill body). **Planning skills never arm it** — `/operationalize-design` and `/generate-tasks` author a phase, so "0/N resolved" is their successful end state, and holding them to "every task COMPLETED" is demanding work they were never asked to do. Every other skill is likewise unarmed. Outside an armed run the Stop hook has no opinion at all: no advisory guard, no inferred phase. Jon's word disarms the gate for **that run only** — his next non-stop prompt lifts the latch, and re-invoking the skill re-arms it. The block budget resets whenever the pending count drops, so `_MAX_BLOCKS` bounds a *wedged* run, never a long healthy one.

Arming used to also fire from the agent's own task-file mutations, so that a run resumed after a compaction stayed enforced. That signal cannot tell **authoring** a task file from **implementing** one — they are the same tool call — so it armed on planning runs too, and one `/operationalize-design` session read the resulting Stop refusal as authorization and began implementing a phase nobody had scheduled. The cost of the narrower rule is known and accepted: a run resumed with "keep going" is unenforced until Jon names the skill again.
...
- **A whole package suite is a phase-boundary act.** Inside a task you run exactly the tests named in its `## Targeted Tests` — `test.ps1 <pkg> -Specific "a.py,b.py"`, batched into one invocation. A bare `test.ps1 <pkg>`, `-All`, `-Rerun`, or a tier sweep (`-Unit`/`-Fast`/…) is a full run and is reserved for the phase-boundary / quality gate, where it happens **once** over the affected set. Running it per task per wave does that work N times for the same information. Using a full suite to *discover* further work mid-task is the same anti-pattern wearing a better excuse — that is what the boundary gate is for. To prove a change generalises, write a test in the owning package; a test proves the invariant forever, a sweep proves it once and evaporates. If a task file's `## Targeted Tests` names a bare full suite, the task file is defective — run the specific files covering the code you changed and say so in your report.

  **This is enforced by the harness, not left to the agent.** `PreToolUse(Bash|PowerShell)` → `guard-full-suite-runs.py` blocks whole-suite `test.ps1` runs. **For subagents the block is unconditional — no marker, no ticket, no override.** The main session may authorize one only by writing `D:\datrix\.tmp\full-suite-ticket.json` (explicit package list or `"*"`, a written reason, `expires_epoch` capped at 6h); every decision, allowed *and* blocked, is appended to `D:\datrix\.tmp\full-suite-audit.jsonl`, so the count of full runs is a fact to read rather than something to notice. `test-single.ps1` and targeted `-Specific`/`-Keyword` runs are never touched. Both directions — block *and* over-block — are covered by `.claude/hooks/test-full-suite-guard.py`. This is a block rather than an instruction because the rule was already written down, was restated in every dispatch, and four subagents ran bare full suites anyway (aws 1844, azure 3294, docker 1880, component 427) inside one phase — and the orchestrator read those reports as reassurance instead of rejecting them.
...
| Hook | Event | Behavior |
|---|---|---|
| `post-compact-context.py` | `SessionStart(compact)` | Injects the execution contract + design-principles cheat sheet **verbatim**, lists the gated docs, reports task files touched in the last 24h. Arms the mandatory-reads gate below (never the orchestration Stop gate); warns loudly if the transcript schema has drifted. |
...
**One exception, and it is exactly one line.** At the very end of a `/task-orchestrator` run (its Step 4), the orchestrator rewrites a design doc's `Status:` line to `Implemented` — and only when every task referencing that doc, across ALL phases and not just the run's, is COMPLETED and the phase passed its test + design-conformance gates. A doc with an outstanding task anywhere is left untouched and reported instead. The body is never edited, no partial/in-progress status is ever written, and nothing about this licenses a mid-run design edit.
