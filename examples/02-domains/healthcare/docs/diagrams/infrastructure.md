...
graph LR
    ...
    patient_service_rdbms_patient_db[("patientDb / postgres (1 entities)")]
    patient_service_cache["redis"]
    patient_service_mq_mq[["mq / MQ"]]
    end
    subgraph "AppointmentService"
    appointment_service_rdbms_appointment_db[("appointmentDb / postgres (1 entities)")]
    appointment_service_cache["redis"]
    appointment_service_mq_mq[["mq / MQ"]]
    appointment_service_jobs[/"Jobs"/]
    end
    subgraph "MedicalRecordService"
    medical_record_service_rdbms_medical_record_db[("medicalRecordDb / postgres (3 entities)")]
