...
graph LR
    ...
    user_service_rdbms_user_db[("userDb / postgres (3 entities)")]
    user_service_cache["redis"]
    user_service_mq_mq[["mq / MQ"]]
    end
    subgraph "ProductService"
    product_service_rdbms_product_db[("productDb / postgres (3 entities)")]
    ...
    order_service_rdbms_order_db[("orderDb / postgres (3 entities)")]
    order_service_cache["redis"]
    order_service_mq_mq[["mq / MQ"]]
    order_service_jobs[/"Jobs"/]
    end
    subgraph "PaymentService"
    payment_service_rdbms_payment_db[("paymentDb / postgres (2 entities)")]
    payment_service_mq_mq[["mq / MQ"]]
    end
    subgraph "ShippingService"
    shipping_service_rdbms_shipping_db[("shippingDb / postgres (3 entities)")]
