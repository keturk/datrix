...
# ---------------------------------------------------------------------------
DEBUG: bool = False
ALLOWED_HOSTS: list[str] = ["payment-service.example.com", "localhost"]
...
class AppSettings:
    ...
    payment_db_host: str
    payment_db_port: int
    payment_db_database: str
    payment_db_user: str
    payment_db_async_driver: str
    payment_db_password_handle: str
    db_pool_size: int
    db_max_overflow: int
...
async def assemble_settings(
    ...
    """
    from ecommerce_payment_service.config.remote_config import ConnectionsKeys

    _payment_db_host = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.PAYMENT_DB_HOST
    )
    _payment_db_port = config_client.get_int(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.PAYMENT_DB_PORT
    )
    _payment_db_database = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.PAYMENT_DB_DATABASE
    )
    _payment_db_user = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.PAYMENT_DB_USER
    )
    ...
    _settings = AppSettings(
        app_name=_bootstrap.PROFILE + "-ecommerce_payment_service",
        environment=_bootstrap.PROFILE,
        host="0.0.0.0",
        port=8003,
        payment_db_host=_payment_db_host,
        payment_db_port=_payment_db_port,
        payment_db_database=_payment_db_database,
        payment_db_user=_payment_db_user,
        payment_db_async_driver="postgresql+asyncpg",
        payment_db_password_handle="payment_db_password",
        db_pool_size=20,
        db_max_overflow=20,
