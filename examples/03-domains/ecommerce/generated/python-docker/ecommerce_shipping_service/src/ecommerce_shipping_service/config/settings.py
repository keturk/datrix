...
from ecommerce_shipping_service.config import _bootstrap
...
class AppSettings:
    ...
    host: str
    port: int
    shipping_db_host: str
    shipping_db_port: int
    shipping_db_database: str
    shipping_db_user: str
    shipping_db_async_driver: str
    shipping_db_password_handle: str
    db_pool_size: int
    db_max_overflow: int
    redis_url: str
...
async def assemble_settings(
    ...
    """
    from ecommerce_shipping_service.config.remote_config import ConnectionsKeys

    _shipping_db_host = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.SHIPPING_DB_HOST
    )
    _shipping_db_port = config_client.get_int(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.SHIPPING_DB_PORT
    )
    _shipping_db_database = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.SHIPPING_DB_DATABASE
    )
    _shipping_db_user = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.SHIPPING_DB_USER
    )
    ...
    _settings = AppSettings(
        app_name=_bootstrap.PROFILE + "-ecommerce_shipping_service",
        environment=_bootstrap.PROFILE,
        host="0.0.0.0",
        port=8005,
        shipping_db_host=_shipping_db_host,
        shipping_db_port=_shipping_db_port,
        shipping_db_database=_shipping_db_database,
        shipping_db_user=_shipping_db_user,
        shipping_db_async_driver="postgresql+asyncpg",
        shipping_db_password_handle="shipping_db_password",
        db_pool_size=20,
        db_max_overflow=20,
        redis_url=redis_url,
