...
from ecommerce_order_service.config import _bootstrap
...
class AppSettings:
    ...
    order_db_host: str
    order_db_port: int
    order_db_database: str
    order_db_user: str
    order_db_async_driver: str
    order_db_password_handle: str
    db_pool_size: int
    db_max_overflow: int
    redis_url: str
    mq_bootstrap_servers: str
    QUEUE_PROCESS_PAYMENT_CONCURRENCY: int
    QUEUE_SEND_ORDER_CONFIRMATION_CONCURRENCY: int
    QUEUE_SETTLE_PAYMENT_CONCURRENCY: int
    QUEUE_HOST: str
    QUEUE_PORT: int
    QUEUE_USER: str
    QUEUE_VHOST: str
...
async def assemble_settings(
    ...
    """
    from ecommerce_order_service.config.remote_config import ConnectionsKeys

    _order_db_host = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.ORDER_DB_HOST
    )
    _order_db_port = config_client.get_int(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.ORDER_DB_PORT
    )
    _order_db_database = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.ORDER_DB_DATABASE
    )
    _order_db_user = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.ORDER_DB_USER
    )
    ...
    ecommerce_product_service_service_url = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.ECOMMERCE_PRODUCT_SERVICE_BASE_URL
    )
    ecommerce_user_service_service_url = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.ECOMMERCE_USER_SERVICE_BASE_URL
    )

    global _settings
    _settings = AppSettings(
        app_name=_bootstrap.PROFILE + "-ecommerce_order_service",
        environment=_bootstrap.PROFILE,
        host="0.0.0.0",
        port=8002,
        order_db_host=_order_db_host,
        order_db_port=_order_db_port,
        order_db_database=_order_db_database,
        order_db_user=_order_db_user,
        order_db_async_driver="postgresql+asyncpg",
        order_db_password_handle="order_db_password",
        db_pool_size=20,
        db_max_overflow=20,
        redis_url=redis_url,
        mq_bootstrap_servers=mq_bootstrap_servers,
        QUEUE_PROCESS_PAYMENT_CONCURRENCY=5,
        QUEUE_SEND_ORDER_CONFIRMATION_CONCURRENCY=5,
        QUEUE_SETTLE_PAYMENT_CONCURRENCY=5,
