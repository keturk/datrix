...
class AppSettings:
    ...
    host: str
    port: int
    notification_db_host: str
    notification_db_port: int
    notification_db_database: str
    notification_db_user: str
    notification_db_async_driver: str
    notification_db_password_handle: str
    db_pool_size: int
    db_max_overflow: int
    QUEUE_SEND_ORDER_CONFIRMATION_CONCURRENCY: int
    QUEUE_HOST: str
    QUEUE_PORT: int
    QUEUE_USER: str
    QUEUE_VHOST: str
    ...
    jwt_audience: str
    jwt_issuer: str
    ecommerce_order_service_service_url: str
...
async def assemble_settings(
    ...
    _notification_db_host = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.NOTIFICATION_DB_HOST
    )
    _notification_db_port = config_client.get_int(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.NOTIFICATION_DB_PORT
    )
    _notification_db_database = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.NOTIFICATION_DB_DATABASE
    )
    _notification_db_user = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.NOTIFICATION_DB_USER
    )
    _queue_host = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.QUEUES_HOST
    )
    _queue_port = config_client.get_int(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.QUEUES_PORT
    )
    ecommerce_order_service_service_url = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.ECOMMERCE_ORDER_SERVICE_BASE_URL
    )

    global _settings
    _settings = AppSettings(
        app_name=_bootstrap.PROFILE + "-ecommerce_notification_service",
        environment=_bootstrap.PROFILE,
        host="0.0.0.0",
        port=8001,
        notification_db_host=_notification_db_host,
        notification_db_port=_notification_db_port,
        notification_db_database=_notification_db_database,
        notification_db_user=_notification_db_user,
        notification_db_async_driver="postgresql+asyncpg",
        notification_db_password_handle="notification_db_password",
        db_pool_size=20,
        db_max_overflow=20,
        QUEUE_SEND_ORDER_CONFIRMATION_CONCURRENCY=5,
        QUEUE_HOST=_queue_host,
        QUEUE_PORT=_queue_port,
        QUEUE_USER="guest",
        QUEUE_VHOST="/",
        ...
        jwt_audience="",
        jwt_issuer="",
        ecommerce_order_service_service_url=ecommerce_order_service_service_url,
    )
