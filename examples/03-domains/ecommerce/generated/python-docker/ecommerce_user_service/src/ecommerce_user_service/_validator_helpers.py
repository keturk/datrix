...
def _validator_validate_datasets(datasets: dict[str, object]) -> dict[str, object]:
    """Validate a collection of parsed datasets.

    Performs basic structural checks: each dataset key must map to a non-empty
    list. Returns a result dict with ``isValid`` flag and ``errors`` list.

    Override this function to add domain-specific schema validation,
    coordinate range checks, or duplicate detection.

    Args:
        datasets: Dict mapping dataset names to lists of parsed records.

    Returns:
        ``{"isValid": bool, "errors": list[str]}``
    """
    errors: list[str] = []
    if not isinstance(datasets, dict):
        return {"isValid": False, "errors": ["datasets must be a dict"]}
    for name, records in datasets.items():
        if not isinstance(records, list):
            errors.append(
                f"Dataset '{name}' must be a list, got {type(records).__name__}"
            )
        elif len(records) == 0:
