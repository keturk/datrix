...
# ---------------------------------------------------------------------------
DEBUG: bool = False
ALLOWED_HOSTS: list[str] = ["user-service.example.com", "localhost"]
...
class AppSettings:
    ...
    user_db_user: str
    user_db_async_driver: str
    user_db_password_handle: str
    db_pool_size: int
    db_max_overflow: int
    redis_url: str
...
async def assemble_settings(
    ...
    _user_db_host = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.USER_DB_HOST
    )
    _user_db_port = config_client.get_int(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.USER_DB_PORT
    )
    _user_db_database = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.USER_DB_DATABASE
    )
    _user_db_user = config_client.get_string(
        ConnectionsKeys.NAMESPACE, ConnectionsKeys.USER_DB_USER
    )
    ...
    _settings = AppSettings(
        app_name=_bootstrap.PROFILE + "-ecommerce_user_service",
        environment=_bootstrap.PROFILE,
        host="0.0.0.0",
        port=8006,
        user_db_host=_user_db_host,
        user_db_port=_user_db_port,
        user_db_database=_user_db_database,
        user_db_user=_user_db_user,
        user_db_async_driver="postgresql+asyncpg",
        user_db_password_handle="user_db_password",
        db_pool_size=20,
        db_max_overflow=20,
        redis_url=redis_url,
