...
import { requestContextMiddleware } from './ecommerce_product_service/_requestContext';
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('ecommerce.ProductService API')
    .setDescription('Auto-generated API documentation for ecommerce.ProductService')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  const document = SwaggerModule.createDocument(app, config, {
    extraModels: [AvailabilityItem, AvailabilityResponse, BulkProductRequest, CheckAvailabilityRequest, ConfirmReservationRequest, CreateProductRequest, ReleaseReservationRequest, ReservationResponse, ReserveInventoryRequest, UpdateInventoryRequest],
  });
