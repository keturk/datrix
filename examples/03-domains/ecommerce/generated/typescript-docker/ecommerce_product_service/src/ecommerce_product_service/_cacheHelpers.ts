...
export async function _redisFlatClearHashPrefix(redis: Redis, prefix: string): Promise<void> {
  const stream = redis.scanStream({ match: `${prefix}:*` });
  for await (const keyBatch of stream) {
    const keys = keyBatch as string[];
    if (keys.length > 0) {
...
export async function _redisFlatHashKeys(redis: Redis, prefix: string): Promise<string[]> {
  const out: string[] = [];
  const head = `${prefix}:`;
  const stream = redis.scanStream({ match: `${prefix}:*` });
  for await (const keyBatch of stream) {
    for (const k of keyBatch as string[]) {
      if (k.startsWith(head)) {
        out.push(k.slice(head.length));
      }
    }
  }
  return out;
