 ...
 *
 * Timeout: 10000ms (override: USER_SERVICE_TIMEOUT_MS)
...
const TIMEOUT_ENV = 'USER_SERVICE_TIMEOUT_MS';
const TIMEOUT_MS = parseInt(process.env[TIMEOUT_ENV] ?? '', 10) || DEFAULT_TIMEOUT_MS;
const SERVICE_URL_ENV = 'USER_SERVICE_SERVICE_URL';
const DEFAULT_URL = 'http://localhost:8000';
const DEPENDENCY_AUDIENCE = 'UserService';
...
@Injectable()
export class UserServiceClientService {
  private readonly logger = new Logger(UserServiceClientService.name);
  ...
  /**
   * Make a resilient HTTP request to UserService.
  ...
  async request(
    ...
    this.logger.debug(`UserService request: ${method} ${url}`);
    ...
    const makeCall = async (): Promise<AxiosResponse> => {
      ...
      try {
        ...
        if (err.name === 'CanceledError' || err.name === 'AbortError') {
          throw new Error(`Request to UserService timed out after ${TIMEOUT_MS}ms`);
        }
        throw error;
      } finally {
