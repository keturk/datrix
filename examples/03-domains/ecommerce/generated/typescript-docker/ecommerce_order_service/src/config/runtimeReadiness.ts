 ...
 * SecretsService from '../ecommerce_order_service/secrets/secrets.service'), proving the live
...
import { SecretResolutionError, SecretsService } from '../ecommerce_order_service/secrets/secrets.service';
...
const REQUIRED_CONFIG_KEYS: Array<[string, string]> = [
  ["ecommerce_product_service_base_url", "ecommerce_product_service_base_url"],
  ["ecommerce_user_service_base_url", "ecommerce_user_service_base_url"],
  ["mq_brokers", "mq_brokers"],
  ["mq_port", "mq_port"],
  ["order_db_database", "order_db_database"],
 ...
 * successful resolution classifies OK; SecretResolutionError classifies
 * MISSING (SecretsService.getSecret is fail-closed -- it throws rather than
 * returning undefined for an absent secret); any other exception classifies
 * ERROR.
 */
async function probeSecret(
  ...
  try {
    await secrets.getSecret(logical);
  } catch (err) {
