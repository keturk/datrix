...
export default () => ({
  ...
  orderDbDatabase: {
    ...
    database: process.env['ORDER_DB_DATABASE'] ?? 'library_books',
    username: process.env['ORDER_DB_USER'] ?? 'postgres',
    password: process.env['ORDER_DB_PASSWORD'] ?? '',
    pool: {
      min: parseInt(process.env['ORDER_DB_POOL_MIN'] ?? '2', 10),
      max: parseInt(process.env['ORDER_DB_POOL_MAX'] ?? '20', 10),
    },
    ssl: (process.env['ORDER_DB_SSL'] ?? 'false') === 'true',
  },
  redisCache: {
