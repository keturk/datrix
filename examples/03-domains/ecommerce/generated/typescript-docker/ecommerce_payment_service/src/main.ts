...
import { requestContextMiddleware } from './ecommerce_payment_service/_requestContext';
import { SwaggerModule, DocumentBuilder } from '@nestjs/swagger';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';
import { RemoteConfigClient } from './config/remoteConfig';
import { ChargeResult } from './dto/charge-result.struct';
...
import { RefundResult } from './dto/refund-result.struct';
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('ecommerce.PaymentService API')
    .setDescription('Auto-generated API documentation for ecommerce.PaymentService')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  const document = SwaggerModule.createDocument(app, config, {
    extraModels: [ChargeResult, ProcessPaymentRequest, RefundPaymentRequest, RefundResult, StripeWebhookRequest],
  });
