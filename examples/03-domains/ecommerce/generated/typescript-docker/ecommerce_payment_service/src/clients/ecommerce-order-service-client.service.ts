 ...
 *
 * Timeout: 10000ms (override: ECOMMERCE_ORDER_SERVICE_TIMEOUT_MS)
...
export class OrderServiceClientService {
  ...
  async request(
    ...
    this.logger.debug(`ecommerce.OrderService request: ${method} ${url}`);
    ...
    const makeCall = async (): Promise<AxiosResponse> => {
      ...
      try {
        ...
        if (err.name === 'CanceledError' || err.name === 'AbortError') {
          throw new Error(`Request to ecommerce.OrderService timed out after ${TIMEOUT_MS}ms`);
        }
        throw error;
      } finally {
