...
export default () => ({
  ...
  paymentDbDatabase: {
    ...
    database: process.env['PAYMENT_DB_DATABASE'] ?? 'library_books',
    username: process.env['PAYMENT_DB_USER'] ?? 'postgres',
    password: process.env['PAYMENT_DB_PASSWORD'] ?? '',
    pool: {
      min: parseInt(process.env['PAYMENT_DB_POOL_MIN'] ?? '2', 10),
      max: parseInt(process.env['PAYMENT_DB_POOL_MAX'] ?? '20', 10),
    },
    ssl: (process.env['PAYMENT_DB_SSL'] ?? 'false') === 'true',
  },
});
