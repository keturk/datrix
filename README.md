...
*Latin: "she who gives" — from dare (to give), the same root as "data" (things given). The -trix suffix is the feminine agent form in Latin, the counterpart to -tor (as in actor, orator). Datrix gives you your entire backend from a simple high-level language.*

## What is Datrix?

Datrix is a backend microservices application code generator. You define your architecture once in a concise Datrix DSL — entities, services, APIs, events, infrastructure — and Datrix generates a complete, production-ready microservices system from it.

**97 lines of `.dtrx` &rarr; 542 files of production code.** That's not a toy demo — that's a fully functional 3-service system with PostgreSQL, REST APIs, Docker Compose, tests, and monitoring.
